...
IF N_Elements(parent) EQ 0 THEN self.bastard = 1

; Create widget
Window, /Free, /Pixmap, xsize = self.size[0], ysize = self.size[0]
self.pix_WID = !D.Window
IF self.bastard THEN self.tlbID = Widget_Base(Column=1, Event_Pro = 'Kang_Window_Events', /KBRD_Focus_Events, UValue = self) $
   ELSE self.tlbID = Widget_Base(parent, Column=1, Event_Pro = 'Kang_Window_Events', /KBRD_Focus_Events, UValue = self)
...
; Bring up window on display
IF self.bastard THEN BEGIN
    Widget_Control, self.tlbID, /Realize
    XManager, 'kang_window', Event_Handler = 'Kang_Window_Events', self.tlbID, /no_block
ENDIF
...
IF N_Elements(self.event_pro) NE 0 THEN Call_Procedure, self.event_pro, event
