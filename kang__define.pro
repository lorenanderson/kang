...
; 3) Enable regions to be resized with the mouse
; 4) Clean up threshold event programs
; 5) Get minispectra plot working again
; 6) Better spectrum display control
; 7) Region names
; 8) All regiongroup properties
;
;+
...
IF self.realized THEN BEGIN
    IF self.view_sliders THEN BEGIN
    ...
    ENDIF
    self.image->GetProperty, filename = filename
    Widget_Control, self.filenamelabelID, Set_Value = filename
...
;    Widget_Control, self.powerID, Set_Button=0
    ...
    Widget_Control, self.percentminmaxID, Set_Button=0
    Widget_Control, self.percent9999ID, Set_Button=0
    Widget_Control, self.percent999ID, Set_Button=0
    Widget_Control, self.percent997ID, Set_Button=0
    Widget_Control, self.percent995ID, Set_Button=0
    Widget_Control, self.percent99ID, Set_Button=0
    Widget_Control, self.percent98ID, Set_Button=0
    Widget_Control, self.percent97ID, Set_Button=0
...
;    The index of the image to delete.  Defaults to current image.
;
; :Keywords:
...
; If all deleted
IF self.image_container->count() EQ 0 AND self.realized THEN BEGIN
   winID = !d.window
   WSet, self.pix_wid
   erase
   self->Copy_Display
...
; :Private:
;-
PRO Kang::SetProperty, noimage = noimage, _extra = extra
self.image->SetProperty, noimage = noimage, _extra = extra
END; Kang::SetProperty---------------------------------------------
...
IF Arg_Present(astr) THEN self.image->GetProperty, astr = astr
...
IF Arg_Present(image) THEN self.image->GetProperty, image = image
...
IF Arg_Present(limits) THEN self.image->GetProperty, levels = levels
...
IF Arg_Present(regionobj) THEN regionobj = self.regions
...
IF Arg_Present(minscale) THEN self.histogram->GetProperty, minscale = minscale
IF Arg_Present(maxscale) THEN self.histogram->GetProperty, maxscale = maxscale
...
;
; :Keywords:
;  coordinates :  in, optional, type = 'string'
;    Set this keyword to the input coordinates
;
; :Examples:
...
;
;-
PRO Kang::Limits, xmin, xmax, ymin, ymax, coordinates = coordinates

IF N_Elements(xmin) EQ 4 THEN limits = xmin ELSE limits = [xmin, xmax, ymin, ymax]

; Change limits
IF ~Obj_Valid(self.image) THEN RETURN
self.image->Limits, limits, coordinates = coordinates
...
; These are arrays
        ...
        self->Load_Image, image, header, filename = arrayname, good_image = good_image, /noplot
    ENDELSE
ENDELSE
...
IF N_Elements(filename_in) EQ 0 AND ~self.realized THEN BEGIN
    print, 'If working from the command line, you must pass a filename string.'
    RETURN
ENDIF
IF N_Elements(filename_in) NE 0 THEN filename = filename_in
...
IF ~Keyword_Set(noSliderUpdate) THEN BEGIN
    IF self.realized AND self.view_sliders THEN BEGIN
...
;Rescale Image
    IF ~Keyword_Set(noupdate) THEN self.image->BytScl, lowScale, highScale, percent = percent
ENDIF
IF ~Keyword_Set(noplot) THEN self->Draw
...
;    Set this to the desired units for velocity.
;    Currently supports "pixel", "m/s", "km/s"
;  whichImage : in, optional, type = 'integer'
...
PRO Kang::Velocity, velocity, velocity2, whichImage = whichImage, $
  deltavel = deltavel, pixel = pixel, noSliderUpdate = nosliderupdate, unit = unit
...
image->Velocity, velocity, velocity2, deltavel=deltavel, pixel=pixel
image->GetProperty, velocity=velocity, deltavel=deltavel, pix_newlimits = pix_newlimits, pix_limits = pix_limits, newlimits = newlimits
print, 'Integrating image around ' + strtrim(velocity, 2) + ' with width ' + strtrim(deltavel, 2)
...
;    self->Draw_Spectra, flag = velocity
ENDIF
...
IF N_Elements(unit) NE 0 THEN BEGIN
   self.image->Set_Property, vel_unit=unit
ENDIF
...
;
;-
PRO Kang::Aper, flux = flux

Widget_Control, Hourglass = 1
self.image->Aper, flux = flux
...
self.image->Filter, sharplim = sharplim, roundlim = roundlim, centroidlim = centroidlim
...
; :Private:
;-
PRO Kang::Filter, sharplim = sharplim, roundlim = roundlim, centroidlim = centroidlim
...
;+
; Sorts the stars found with "find" by the various parameters
;
; :Private:
;-
PRO Kang::Sort_Stars, x = x, y = y, flux = flux, roundness = roundness, sharpness = sharpness

self.image->Sort_Stars, x = x, y = y, flux = flux, roundness = roundness, sharpness = sharpness
self->Update_DAOphot_Widget
...
;
;-
PRO Kang::Sky, skyval, skysig, CIRCLERAD = circlerad, MEANBACK = meanback, _EXTRA = extra

self.image->Sky, skyval, skysig, CIRCLERAD = circlerad, MEANBACK = meanback, _EXTRA = extra
self->Update_DAOPhot_Widget
...
IF N_Elements(position) EQ 0 THEN position = selected_regions
...
IF N_Params() EQ 2 THEN BEGIN
    IF Keyword_Set(pixel) THEN BEGIN
        pix_xy = [x, y]
        newcoord = self.image->Convert_Coord(x, y, old_coords = 'Image')
    ENDIF ELSE BEGIN
...
; Create spectrum
    ...
    newcoord = self.image->Convert_Coord(xrange, yrange, old_coord = 'Image')
    Widget_Control, Hourglass=0

    title = 'Spectrum of ('+StrTrim(newcoord[0], 2)+': '+StrTrim(newcoord[2], 2)+$
            ',  '+StrTrim(newcoord[1], 2)+': '+StrTrim(newcoord[3], 2)+')    N = ' + $
...
;    bpix = 0                    ;convert_array[3]
    l_gal=newcoord[0]           ;(lpix - lmap0)*delta_l   
    b_gal=newcoord[1]           ;(bpix - bmap0)*delta_b 
    euler, l_gal, b_gal, ra, dec, 2
...
;    !b[0].ver_offset=bpix
    ...
    !b[0].data[0:N_Elements(yvals)-1]=yvals
...
;
;-
PRO Kang::Draw_Spectra, flag = flag
...
IF N_Elements(flag) NE 0 THEN BEGIN
   ...
   IF n_spectra[0] NE -1 AND XRegistered('plot_windows '+StrTrim(self.tlbID, 2)) THEN BEGIN
      
;        Kang_Draw_Spectra, tlbID
      Plots, [flag, flag], !y.crange, color = 240
; velchange
    ENDIF
ENDIF
...
; Easier just to do it here
    ...
    IF strmid(filename[0], 0, 1) EQ '!' THEN DefSysV, filename[0], image ELSE $
      (SCOPE_VARFETCH(filename[0], /ENTER, LEVEL=1)) = yvals
    IF strmid(filename[1], 0, 1) EQ '!' THEN DefSysV, filename[1], hdr ELSE $
      (SCOPE_VARFETCH(filename[1], /ENTER, LEVEL=1)) = xvals
    RETURN
...
; Configure the PostScript device.
        Set_Plot, 'PS'
        Device, filename = filename, _Extra=keywords
;        !P.Charsize = 1.75
;        !P.Thick = 2
;        !X.Thick = 2
;        !Y.Thick = 2
...
;+
; Brings up the smoothing dialog
;
; :Private:
;
;-
PRO Kang::Smooth_Widget

IF XRegistered('Smooth '+StrTrim(self.tlbID, 2)) THEN RETURN

; Get smoothing radius, type
self.image->GetProperty, smooth_radius = smoothing_radius, smooth_type = smoothing_type

; Create widget bases
tlb = Widget_Base(Column=1, /TLB_Size_Events, Title='Smooth Parameters', UValue = self, /Base_Align_Center, tlb_frame_attr=1)
subBase = Widget_Base(tlb, Column=1, Frame=1)

; Create list of smoothing types
types = ['Boxcar', 'Median', 'Gaussian']
smoothTypeID = CW_BGroup(subBase, types, Set_Value = (where(types EQ smoothing_type))[0], /Row, /Exclusive)

; Radius slider
sliderBase = Widget_Base(subBase, Row = 1)
label = Widget_Label(sliderBase, Value = 'Radius: ')
radius_sliderID = Widget_Slider(sliderBase, min = 1, max = 20, xsize = 120, Value = smoothing_radius)

; Apply Button
butbase = Widget_Base(tlb, Row=1, /Align_Center)
acceptID = Widget_Button(butbase, Value='  Apply  ', UValue = 'Axis Apply')

; Create structure with IDs
UVal = {self:self, $
       smoothTypeID:smoothTypeID, $
       radius_sliderID:radius_sliderID}

; Center the program on the display
centertlb, tlb

; Realize top-level base and all of its children.
Widget_Control, tlb, Set_Uval = uval, /Realize
XManager, 'Smooth '+StrTrim(self.tlbID, 2), tlb, /No_Block, Event_Handler = 'Kang_Smooth_Events', Group_Leader=self.tlbID

END; Kang_Smooth_Widget---------------------------------------
...
;
;-
PRO Kang::Smooth, radius, type = type

self.image->Smooth, radius, type = type

; Update widget
;Widget_Control, self.smooth_widgetID, Get_UVal = uval
;types = ['Boxcar', 'Median', 'Gaussian']
;Widget_Control, uval.smoothing_typeID, Set = where(type EQ types)
;Widget_Control, uval.smoothing_radiusID, Set = radius
...
;+
; Creates a model background through 2D polynomial fitting.  Areas to
; include in the fit can be set by high and low pass filters.
;
; :Keywords:
;  background : out, optional, type = 'float array'
;    The computed background map
;  all : in, optional, type = 'boolean'
;    If set, a mask will be computed from all regions.  This will by
;    default INCLUDE all pixels within regions in the background computation
;  high_filter : in, optional, type = 'float'
;    Image values above this will not be included in the fit
;  inverted : in, optional, type = 'boolean'
;    If set, the opposite of the computed mask will be used.  If a
;    mask is input via the mask keyword, this keyword will not affect it.
;  low_filter : in, optional, type = 'float'
;    Image values below this will not be included in the fit
;  mask : in, optional, type = 'boolean array'
;    Input mask to use when computing the background.  Must be same
;    size as image.  Values of 1 will be uncluded in the background
;    modelling and those with 0 will not be.
;  order : in, optional, type = 'integer'
;    The order of the 2D polynomial to fit
;  selected : in, optional, type = 'boolean'
;    If set, a mask will be computed from selected regions.  This will by
;    default INCLUDE all pixels within selected regions in the background
;    computation
;
;-
PRO Kang::Background, background = background, order = order, $
                      high_filter = high_filter, low_filter = low_filter, mask = mask, $
                      all = all, selected = selected, inverted = inverted

IF (Keyword_Set(all) OR Keyword_Set(selected)) AND N_Elements(mask) EQ 0 THEN $
   mask = self->Get_Mask(all = all, selected = selected)

; Compute background
self.image->background, background = background, order = order, $
                        high_filter = high_filter, low_filter = low_filter, mask = mask
...
;
;-
PRO Kang::Subtract_Background

self.image->Subtract_Background

END; Kang::Subtract_Background--------------------------------------------
...
;
;-
PRO Kang::Velocity_Field, output = output, velrange = velrange

self.image->Velocity_Field, output = output, velrange = velrange

END; Kang_Velocity_Field--------------------------------------------------
...
PRO Kang::Transpose, lbv = lbv, bvl = bvl, vbl = vbl, lvb = lvb, vlb = vlb

self.image->Transpose, lbv = lbv, bvl = bvl, vbl = vbl, lvb = lvb, vlb = vlb

; Update Velocity widget
...
; Create regions
IF self.realized THEN  Widget_Control, self.tlbID, Hourglass=1 ; This may take a while
...
IF self.realized THEN Widget_Control, self.tlbID, Hourglass=0
...
;--------------------------------------------------
;+
; Copies selected regions to clipboard but retains them on current
; display.
;
;-
PRO Kang::Copy
...
FOR i = 0, n_selected-1 DO BEGIN
    regions[i]->GetProperty, regiontype = regiontype, params = params, cname = color, thick = width, linestyle = linestyle, $
      source = source, interior = exclude, polygon = polygon, coordinate = coordinate
    ...
    IF StrUpCase(regiontype) EQ 'THRESHOLD' THEN BEGIN
        ...
        regiontype = 'Polygon'
    ENDIF
    (*self.copied_regions)[i] = Ptr_New({regiontype:regiontype, params:params, coords:coordinate, color:color, width:width, $
...
;+
; Copies selected regions to clipboard and deletes them from current
; display.
;
;-
PRO Kang::Cut
...
;+
; Pastes regions from clipboard (previously copied or cut) into
; current display.
;
;-
PRO Kang::Paste
...
; Create the new regions
        self->Make_Region, (*(*self.copied_regions)[i]).regiontype, (*(*self.copied_regions)[i]).params, color = (*(*self.copied_regions)[i]).color, $
                           ...
                           coordinates = (*(*self.copied_regions)[i]).coords, /noplot
    ENDFOR
ENDIF
...
self.regions->Combine, nostats = self.nostats
self->Draw
self->Update_Regions_Info, /List_Update
...
;+
; Dissolves a selected composite region into its constituent regions.
;
;-
PRO Kang::Dissolve
...
;+
; Returns a mask of pixel values from the loaded regions.  By default,
; this mask is zero if there is no region and 1 if there is.  If the
; bitwise operator is set, region pixels are individually labeled with
; 2, 4, 8, 16, etc., and overlapping regions have additions of these values.
; If the numbered operator is set, region pixel values just increase
; with region 1 pixels having a value of 1, region 2 of 2, etc.  In
; this case the overlap regions are overwritten by the subsequent regions.
;
; :Keywords:
;  all : in, optional, type = 'boolean'
;    If set, all regions will be used.  This is the default.
;  bitwise : in, optional, type = 'boolean'
;    If set, pixels will have values of 2^(i+1), where i is the region index.
;  inverted : in, optional, type = 'boolean'
;    If set, the inverse of the mask will be returned, i.e. pixels
;    normally included will be excluded.
;  numbered : in, optional, type = 'boolean'
;    If set, pixels will have values of i+1, where i is the region index.
;  selected : in, optional, type = 'boolean'
;    If set, only selected regions will be used.
;  weighted : in, optional, private, type = 'integer'
;    If set, the mask values will be weighted by the fraction of each pixel that
;    the region lies inside.  NOT YET IMPLEMENTED.
;
;-
FUNCTION Kang::Get_Mask, all = all, bitwise = bitwise, inverted = inverted, numbered = numbered, selected = selected, position = position, weighted = weighted

; Get Mask
RETURN, self.regions->ComputeMask(all = all, bitwise = bitwise, inverted = inverted, numbered = numbered, position = position, selected = selected, weighted = weighted)
END; Kang::Get_Mask--------------------------------------------------------
...
;
; :Params:
;    position : The zero-based index to use when returning aperture statistics.
;
; :Returns:
...
;   MIN             FLOAT
;   MEAN            FLOAT
;   CENTROID_MIN    FLOAT(2)
;   CENTROID_MAX    FLOAT(2)
;   STDDEV          FLOAT
;   TOTAL           FLOAT
;
...
;
; :Examples:
;   To get the statistics for all selected regions::
;      IDL> stats = kang_obj->Get_Stats()
;
;   To get the statistics for the first and third regions only:
;      IDL> stats = kang_obj->Get_Stats([0,2])
;
;-
FUNCTION Kang::Get_Stats, position

RETURN, self.regions->Get_Stats(position)
END; Kang::Get_Stats--------------------------------------------------
...
; :Private:
;-
FUNCTION Kang::Find_Values, position

indices = self.regions->Find_Indices(position = position)
RETURN, self.image->Find_Values(indices = indices)
END; Kang::Find_Values-------------------------------------------------

;+
; Returns background subtracted aperture photometry measurements.
;
; :Keywords:
;   avg_bg : in, optional, type = 'string'
;     When multiple backgrounds are selected, the background value can be computed
;     by averaging all the separate backgrounds, or by using one background
;     aperture for each source.  Set this for the former, default is the latter.
;   recompute : in, optional, type = 'boolean'
;     Set to use previous apertures and recompute the photometry.  For example,
;     if previously run as apphot(0, 1) but aperture 0 has changed, run with
;     this keyword to update the results
;
; :Params:
;   source : in, optional, type = 'integer'
;     The source aperture to use, specified as the zero-based index.
;   background : in, optional, type = 'integer, float'
;     The background aperture to use, specified as the zero-based index.
;     If input as a float, however, this value will be used.
;     There must be one background for each source, or one background
;     for all sources.
;
; :Returns:
;   A structure with the following tags:
...
; :Examples:
;-
FUNCTION Kang::Apphot, source, background, avg_bg = avg_bg, recompute = recompute

; Recompute will use the previously defined regions
IF Keyword_Set(recompute) THEN BEGIN
   source = (*self.apphot).source
   background = (*self.apphot).background
   selected = (*self.apphot).selected
ENDIF ELSE selected = 0

; Make sure there are regions loaded
self.regions->GetProperty, n_regions = n_regions
IF n_regions EQ 0 THEN BEGIN
   print, 'No regions have been loaded!  Returning.'
   RETURN, [-1]
ENDIF

; Must select bacground
IF N_Elements(background) EQ 0 THEN BEGIN
   print, 'Must select background aperture. Returning.'
   RETURN, [-1]
ENDIF

; Make sure background definition makes sense
IF N_Elements(background) GT 1 AND ~Keyword_Set(avg_bg) AND N_Elements(background) NE N_Elements(source) THEN BEGIN
   print, 'Ambiguous background definition.  Please use one background for each source, one background for all sources, ' + $
          ' or use the "avg_bg" keyword.  Returning.'
   RETURN, [-1]
ENDIF

; Get source flux
IF size(source, /type) EQ 4 OR size(source, /type) EQ 5 THEN source = long(source)
IF N_Elements(source) NE 0 THEN source_flux = self->Get_Stats(source) ELSE source_flux = self->Get_Stats()
filename = source_flux.filename
flux = source_flux.total
npix = source_flux.npix
n_nan = source_flux.n_nan
max = source_flux.max
min = source_flux.min
regiontext = source_flux.regiontext

; Get background flux
; If specified as a float, use contant value for BG
; This seems like a strange way to do things, but ok
IF size(background, /type) EQ 4 OR size(background, /type) EQ 5 THEN BEGIN
   background_flux = {mean:background, $
                      stddev:0, $
                      npix:1., $
                      regionText: ''}
ENDIF ELSE BEGIN
; Get background flux
   background_flux = self->Get_Stats(background)
ENDELSE
bgnpix = background_flux.npix
bgregiontext = background_flux.regiontext

; If multiple background regions are specified, we can average them or not
IF Keyword_Set(avg_bg) THEN BEGIN
   avg_bgflux = avg(background_flux.mean) ; this just averages the two values.  Could also weight by num pix.
   bg_npix = total(background_flux.npix)
   bgval = avg_bgflux / float(bg_npix)
ENDIF ELSE bgval = background_flux.mean

; Compute errors
;self.image->GetProperty, skyval=skyval
;err = skyval*npix^0.5
;err_bgval = skyval*bgnpix^0.5 / bgnpix
IF ~Keyword_Set(avg_bg) THEN BEGIN
   err = background_flux.stddev*npix^0.5
   err_bgval = background_flux.stddev*npix^0.5 / bgnpix
ENDIF ELSE BEGIN
   err = 0
   err_bgval = 0
ENDELSE

; Need to replicate backgrounds to make arrays like that for the sources
IF N_Elements(background) EQ 1 AND ~Keyword_Set(avg_bg) AND N_Elements(source) NE 1 THEN BEGIN
   n_source = N_Elements(source)
   bgval = replicate(bgval, n_source)
   err_bgval = replicate(err_bgval, n_source)
   bgnpix = replicate(bgnpix, n_source)
ENDIF

; Create background subtracted flux
bgsub = flux - npix * bgval
err_bgsub = err + npix * err_bgval

; Create apphot structure
apphot = {filename:filename, $
          selected:selected, $
          avg_bg:Keyword_Set(avg_bg), $
          source:source, $
          background:background, $
          regionText:regionText, $
          bgregionText:bgregionText, $
          flux:flux, $
          max:max, $
          min:min, $
          err:err, $
          ...
          bgsub:bgsub, $
          err_bgsub:err_bgsub}
Ptr_Free, self.apphot
self.apphot = Ptr_New(apphot)

; Create or update Apphot widget
self->Apphot_Widget
self->Update_Apphot_Widget
...
;
;-
PRO Kang::Apphot_Widget

IF ~self.realized THEN RETURN

;  Bring up new dialog if not already on screen
IF XRegistered('Apphot Dialog ' + StrTrim(self.tlbID, 2)) THEN BEGIN
  self->Update_Apphot_Widget
  RETURN
ENDIF

; Top level
tlbID = Widget_Base(row = 5, Title = 'Photometry', UValue = self)
self.apphot_WidgetID = tlbID

; IDs of source/BG
IDsBase = Widget_Base(tlbID, column = 2, frame=1)
label = Widget_Label(IDsBase, Value = 'Sources : ', YOffset = -50)
self.source_dropID = Widget_ComboBox(IDsBase, XSize = 110, UValue = 'Source Droplist')

; Source parameters
source_Base = Widget_Base(tlbID, column = 2, frame=1)
label = Widget_Label(source_Base, Value = 'Flux: ', xsize = 60, /Align_Left)
label = Widget_Label(source_Base, Value = 'Err: ', xsize = 60, /Align_Left)
label = Widget_Label(source_Base, Value = 'NPix: ', xsize = 60, /Align_Left)
self.fluxID = Widget_Label(source_Base, Value = ' ', xsize = 80, /Align_Right)
self.errID = Widget_Label(source_Base, Value = ' ', xsize = 80, /Align_Right)
self.NPixID = Widget_Label(source_Base, Value = ' ', xsize = 80, /Align_Right)

; BG parameters
bg_Base = Widget_Base(tlbID, column = 2, frame=1)
label = Widget_Label(bg_Base, Value = 'BG: ', xsize = 60, /Align_Left)
label = Widget_Label(bg_Base, Value = 'BG Value: ', xsize = 60, /Align_Left)
label = Widget_Label(bg_Base, Value = 'Err: ', xsize = 60, /Align_Left)
label = Widget_Label(bg_Base, Value = 'NPix: ', xsize = 60, /Align_Left)
self.bgID = Widget_Label(bg_Base, Value = ' ', xsize = 80, /Align_Right)
self.bgvalID = Widget_Label(bg_Base, Value = ' ', xsize = 80, /Align_Right)
self.bgvalerrID = Widget_Label(bg_Base, Value = ' ', xsize = 80, /Align_Right)
self.bgNPixID = Widget_Label(bg_Base, Value = ' ', xsize = 80, /Align_Right)

; BGSUB
bgsub_Base = Widget_Base(tlbID, column = 2, frame=1)
label = Widget_Label(bgsub_Base, Value = 'BG Sub.: ', xsize = 60, /Align_Left)
label = Widget_Label(bgsub_Base, Value = 'Err: ', xsize = 60, /Align_Left)
self.bgsubID = Widget_Label(bgsub_Base, Value = ' ', xsize = 80, /Align_Right)
self.bgsuberrID = Widget_Label(bgsub_Base, Value = ' ', xsize = 80, /Align_Right)

; Buttons
recomputeID = Widget_Button(tlbID, Value = ' Recompute ', UValue = 'Recompute')
;writeID = Widget_Button(tlbID, Value = 'Write to File', Event_Pro = '')

; Update
self->Update_Apphot_Widget

; Realize top-level base and all of its children.
IF !D.Name NE 'PS' THEN Widget_Control, tlbID, /Realize

XManager, 'Apphot Dialog ' + StrTrim(self.tlbID, 2), tlbID, /No_Block, Event_Handler = 'Kang_Plot_Params_Events', $
    Group_Leader=self.tlbID, Cleanup = 'Kang_Plot_Params_Events'

END; Kang::Apphot_Widget---------------------------------


PRO Kang::Update_Apphot_Widget, selected

; If aperture photometry has been run, pointer will be valid
IF ~Ptr_Valid(self.apphot) THEN RETURN

; Selected keyword passed from events routine
IF N_Elements(selected) NE 0 THEN (*self.apphot).selected = selected[0] > 0
apphot = *self.apphot

;  Bring up new dialog if not already on screen
IF ~XRegistered('Apphot Dialog ' + StrTrim(self.tlbID, 2)) THEN RETURN

; Turn off updating
Widget_Control, self.source_dropID, Set_Value = [String(apphot.source)], Set_Combobox_Select = apphot.selected
Widget_Control, self.fluxID, Set_Value = String(apphot.flux[apphot.selected])
Widget_Control, self.errID, Set_Value = String(apphot.err[apphot.selected])
Widget_Control, self.npixID, Set_Value = String(apphot.npix[apphot.selected])
Widget_Control, self.bgvalID, Set_Value = String(apphot.bgval[apphot.selected])
Widget_Control, self.bgvalerrID, Set_Value = String(apphot.err_bgval[apphot.selected])
Widget_Control, self.bgnpixID, Set_Value = String(apphot.bgnpix[apphot.selected])
Widget_Control, self.bgsubID, Set_Value = String(apphot.bgsub[apphot.selected])
Widget_Control, self.bgsuberrID, Set_Value = String(apphot.err_bgsub[apphot.selected])

; Turn on updating

END; Kang::Update_Apphot_Widget--------------------------
...
;  fractional : in, optional, type = 'boolean'
;    Set this keyword to calculate photometry using fractional pixels
...
;    If set, stats will not be computed
;  pointType : in, optional, type = 'string'
;    Set to have future point regions drawn with this shape
...
;  whole : in, optional, type = 'boolean'
;    Set this to compute only whole pixels - fractional pixels taken
...
IF N_Elements(pointType) NE 0 THEN BEGIN
   IF self.realized THEN BEGIN
      Widget_Control, self.CirclePointID, Set_Button=0
      Widget_Control, self.PlusPointID, Set_Button=0
      Widget_Control, self.XPointID, Set_Button=0
      CASE CapFirstLetter(pointType) OF
         'Circle': Widget_Control, self.circlePointID, Set_Button=1
         'Plus': Widget_Control, self.plusPointID, Set_Button=1
         'X': Widget_Control, self.xPointID, Set_Button=1
      ENDCASE
   ENDIF
   regionType = 'Point'
   self.pointType = CapFirstLetter(pointType)
ENDIF
...
; Turn checkmark off on all buttons
        ...
        Widget_Control, self.RowID, Set_Button=0
        Widget_Control, self.ColumnID, Set_Button=0
        Widget_Control, self.PointID, Set_Button=0
;        Widget_Control, self.PolygonID, Set_Button=0
;        Widget_Control, self.ProjectionID, Set_Button=0

        CASE CapFirstLetter(regionType) OF 
; Set region type checkmark accordingly
            ...
            'Row': Widget_Control, self.rowID, Set_Button=1
            'Column': Widget_Control, self.columnID, Set_Button=1
            'Point':  Widget_Control, self.pointID, Set_Button=1
;            'Projection': Widget_Control, self.ProjectionID, Set_Button=1
            ELSE: RETURN
        ENDCASE
     ENDIF
...
; Bring up dialog
    IF colorName EQ 1B THEN colorName = pickColorName(self.regions_cName, group_leader = self.tlbID, $
                                                      rows = 6, cnames = *self.colornames)
...
IF Keyword_Set(fractional) THEN BEGIN
   self.regions_fractional = 1B
   IF self.realized THEN BEGIN
       Widget_Control, self.fractionalID, Set_Button = 1B
       Widget_Control, self.wholeID, Set_Button = 0B
   ENDIF
ENDIF
IF Keyword_Set(whole) THEN BEGIN
    self.regions_fractional = 0B
    IF self.realized THEN BEGIN
        Widget_Control, self.wholeID, Set_Button = 1B
        Widget_Control, self.fractionalID, Set_Button = 0B
    ENDIF
ENDIF
...
; Turn on correct checkmark
    IF self.realized THEN BEGIN
        ...
        Widget_Control, self.widthIDs[(Round(width)-1) > 0], Set_Button=1
    ENDIF
ENDIF
...
; Nostats
IF N_Elements(nostats) THEN self.nostats = byte(nostats)
...
;  charthick : in, optional, type = 'integer'
;    The font thickness of the associated text 
;  color : in, optional, type = 'string'
;    Set to a color string
;  coords : in, optional, type = 'string'
...
;    The IDL font to use
;  fractional : in, optional, type = 'boolean'
;    Set to calculate photometry with fractional pixels
...
;    Set to desired display width
;  whole : in, optional, type = 'boolean'
;    Set to calculate photometry with whole pixels only
;
; :Examples:
...
IF self.image_container->count() EQ 0 THEN BEGIN
...
IF N_Elements(coordinates) EQ 0 THEN self.image->GetProperty, current_coords = coordinates
IF ~Keyword_Set(source) AND ~Keyword_Set(background) THEN source = 1B
IF N_Elements(background) NE 0 THEN source = ~background    
IF ~Keyword_Set(fractional) AND ~Keyword_Set(whole) THEN fractional = 1B
IF N_Elements(whole) NE 0 THEN fractional = ~whole    
IF N_Elements(color_in) EQ 0 THEN color = self.regions_color ELSE BEGIN
    cname = color_in
    color = self.colors->ColorIndex(cname)
ENDELSE
...
IF N_Elements(nostats) EQ 0 THEN nostats = self.nostats

CASE StrUpCase(regiontype) OF
    'BEAM': BEGIN
       IF n_par EQ 3 THEN params = [params, 5] ; default to 5 lines
...
; Create array of angles
       theta = asin(s*findgen(max+1) / r)+!pi/4.

; Top half
       xy = cv_coord(from_polar = transpose([[theta], [replicate(r, N_Elements(theta))]]), /to_rect)
       xy2 = cv_coord(from_polar = transpose([[3*!pi/2. - theta], [replicate(r, N_Elements(theta))]]), /to_rect)
       FOR i = 0, max-1 DO self->Make_Region, 'line', [[xy[0, i]+xcenter, xy[1, i]+ycenter], [xy2[0, i]+xcenter, xy2[1, i]+ycenter]], /noplot, $
                                              width = width, linestyle = linestyle, color = color_in

; Bottom half
       xy = cv_coord(from_polar = transpose([[!pi/2 - theta], [replicate(r, N_Elements(theta))]]), /to_rect)
       xy2 = cv_coord(from_polar = transpose([[!pi + theta], [replicate(r, N_Elements(theta))]]), /to_rect)
       FOR i = 1, max-1 DO self->Make_Region, 'line', [[xy[0, i]+xcenter, xy[1, i]+ycenter], [xy2[0, i]+xcenter, xy2[1, i]+ycenter]], /noplot, $
                                              width = width, linestyle = linestyle, color = color_in

; Make circle
       self->Make_Region, 'circle', [50, 50, 20], /noplot, color = color_in, width = width, linestyle = linestyle

; Create composite
       self.regions->GetProperty, n_regions = n_regions
       self->Select, findgen(max*2+2)+n_regions-(max*2+2)
       self->Combine

; Turn off object creation
       noobject = 1
    END
    ...
    'CIRCLE': BEGIN
        data = Kang_Circle(params[0], params[1], params[2], coordinates = coordinates)
        angle = 0
    END
    ...
    'ROW': BEGIN
       self.image->getproperty, limits = limits
       data = [[limits[0], params], [limits[1], params]]
; Convert to data, then back to pixels
       data = round(self.image->Convert_Coord(data, new_coord = 'Image'))+[[-0.5, 0], [0.5, 0]]
       data = self.image->Convert_Coord(data, old_coord = 'Image')
       polygon = 0
       angle = 0
    END
    ...
    'COLUMN': BEGIN
       self.image->getproperty, limits = limits
       data = [[params, limits[2]], [params, limits[3]]]
; Convert to data, then back to pixels
       data = round(self.image->Convert_Coord(data, new_coord = 'Image'))+[[0, -0.5], [0, 0.5]]
       data = self.image->Convert_Coord(data, old_coord = 'Image')
       polygon = 0
       angle = 0
    END
    ...
    'POINT': BEGIN
       ...
       polygon = 0
       angle = 0
       IF N_Elements(pointType) EQ 0 THEN pointType = self.pointType
    END
    ...
    'THRESHOLD': BEGIN
        ...
        IF ~Keyword_Set(noobject) THEN BEGIN
            obj = self.image->Threshold_Region(data[0], data[1], params[2], params[3], $
                                               ...
                                               color = color, cname = cname, font = font, interior = ~include, $
...
IF StrUpCase(regiontype) NE 'THRESHOLD' AND StrUpCase(regiontype) NE 'BEAM' THEN BEGIN
; Convert from data to image coordinates if necessary
    IF data[0] EQ -1 THEN RETURN
    data = self.image->Convert_Coord(data[0, *], data[1, *], old_coords = coordinates, new_coords = 'Image')

; Create object
    IF ~Keyword_Set(noobject) THEN BEGIN
        obj = Obj_New('Kang_Region', data, params = params, $
                      angle = angle,  charsize = charsize, charthick = charthick, $
                      color = color, coordinate = coordinates, cname = cname, font = font, interior = ~include, $
                      image_obj = self.image, regiontype = CapFirstLetter(regiontype), text = text, source = source, $
                      thick = width, linestyle = linestyle, pointType = pointType, polygon = polygon, nostats = nostats, name = name)
    ENDIF
ENDIF
...
;+
; Move previously regions in x and y.
;
; :Params:
;  translation : in, required, type = 'float[2]'
;    The coordinates (x, y) of the translation.  For example, to move in x,
;    translation = [#, 0], and for y, translation = [0, #]
;
; :Keywords:
;  coordinate : in, optional, type = 'string'
;    The coordinates in which to make the translation.  Default is the current image coordinates.
;  position : in, optional, type = 'integer array'
;    The zero-based indices of the regions to move
;
;-
PRO Kang::Move_Region, translation, coordinate = coordinate, position = position
...
; Move regions
self.regions->Move, translation, coordinate = coordinate, position = position
...
;
;-
PRO Kang::Order_Region, regions_to_move, destination = destination, tofront = tofront, toback = toback, $
  forward = forward, backward = backward

self.regions->Order, regions_to_move, destination = destination, tofront = tofront, toback = toback, $
  forward = forward, backward = backward

self->Update_Regions_Info, /List_Update
END;Kang::Order_Region---------------------------------------------
...
;  position : in, optional, type = 'integer'
;    Region index to find indices for.  Defaults to current selection.
;  bins : out, optionsal, type = 'float array'
;    x-values of histogram
;  _Extra : in, optional
;    Use this keyword to pass any additional keywords to the
;    histogram routine.
;
;-
FUNCTION Kang::Histogram_Region, position = position, bins = bins, reverse_indices = reverse_indices, _extra = extra
...
;self.image->PlotHist, indices, XTitle = 'Pixel Value', YTitle = 'Number', charsize = 1, _ref_extra = ref_extra
;self.plot_window->Update
histvals = self.regions->Histogram(position, bins = bins, reverse_indices = reverse_indices, _extra = extra)
RETURN, histvals
...
;    Set this keyword for background regions
;  color : in, optional, type = 'string'
;    Set to a color string.  If set as /colors then a dialog is brought up.
;  include : in, optional, type = 'boolean'
;    Set this keyword for included regions
;  exclude : in, optional, type = 'boolean'
;    Set this keyword for excluded regions
...
;    Set to desired display width, 1 to10
;  width : in, optional, type = 'integer'
;    Same as thick
;
;-
...
; Bring up dialog if needed
    IF size(cName, /tname) NE 'STRING' THEN BEGIN
        IF cName EQ 1 THEN BEGIN
            ...
            cName = pickColorName(CapFirstLetter(region_cName), group_leader = self.tlbID, $
                                  Rows = 4, cNames = *self.colornames)
        ENDIF
    ENDIF
    color = self.colors->ColorIndex(cname)
...
IF N_Elements(thick) NE 0 THEN self.regions->SetProperty, width = thick, position = position
...
IF N_Elements(fractional) NE 0 THEN self.regions->SetProperty, weighted = fractional, position = position
IF N_Elements(whole) NE 0 THEN self.regions->SetProperty, weighted = ~whole, position = position
...
;    Output variable for mask header
;  inverted : in, optional, type = 'boolean'
;    If set, the inverse of the mask will be returned, i.e. pixels
;    normally included will be excluded.
;  mask : out, optional, type = 'byte'
;    Output variable for mask
;  selected : in, optional, type = 'boolean'
;    If set, only selected regions will be used.
;  weighted : in, optional, private, type = 'integer'
;    If set, the mask values will be weighted by the fraction of each pixel that
...
;
;-
PRO Kang::Save_Mask, filename, all = all, header = header, inverted = inverted, mask = mask, selected = selected, weighted = weighted

; Bring up dialog if no filename was passed
IF N_Elements(filename) EQ 0 AND ~Arg_Present(mask) THEN BEGIN
   IF self.realized THEN BEGIN
   ...
   ENDIF ELSE print, 'ERROR: Filename or mask keyword must be specified.'
ENDIF

; Get mask
mask = self->Get_Mask(all = all, inverted = inverted, selected = selected, weighted = weighted)

; Write file
IF N_Elements(filename) NE 0 THEN BEGIN
; Test to see if possible to write
   IF ~File_Test(filename, /Write) THEN BEGIN
      print, 'ERROR: File ' + filename + ' cannot be written.'
      RETURN
   ENDIF
   self.image->GetProperty, header = header
...
;    The name of the regions to select.  Case insensitive!
...
self.regions->Select, positions, Add = add, All = all, Include = include, Exclude = exclude, invert = invert, name = name, none = none, $
...
;+
; Since threshold regions are saved as polygons, this routine will
...
self.image->Limits, pix_newlimits, coordinates = 'Image'
...
;--------------------------------------------------------------------------------
;                Widget definition and update methods
;--------------------------------------------------------------------------------
;+
...
; Widgets
dialogsID = Widget_Button(fileID, Value = 'Dialogs', /Menu)
fileDialogID = Widget_Button(dialogsID, Value='Files')
plot_parametersID = Widget_Button(dialogsID, Value='Plot Parameters')
self.velocity_widgetID = Widget_Button(dialogsID, Value='Velocity')

; Define the print fits header menu
headerID = Widget_Button(fileID, Value='Show FITS Header')

; Save object reference
saverefID = Widget_Button(fileID, Value = 'Save Object Reference')
...
self.percentminmaxID = Widget_Button(self.scalingID, Value = 'Min/Max', /Checked_Menu, /separator)
self.percent9999ID = Widget_Button(self.scalingID, Value = '99.99%', /Checked_Menu)
self.percent999ID = Widget_Button(self.scalingID, Value = '99.9%', /Checked_Menu)
self.percent997ID = Widget_Button(self.scalingID, Value = '99.7%', /Checked_Menu)
self.percent995ID = Widget_Button(self.scalingID, Value = '99.5%', /Checked_Menu)
self.percent99ID = Widget_Button(self.scalingID, Value = '99%', /Checked_Menu)
self.percent98ID = Widget_Button(self.scalingID, Value = '98%', /Checked_Menu)
self.percent97ID = Widget_Button(self.scalingID, Value = '97%', /Checked_Menu)
self.percent95ID = Widget_Button(self.scalingID, Value = '95%', /Checked_Menu)
self.scale_to_imageID = Widget_Button(self.scalingID, Value = 'Scale to Image', /Checked_Menu, /Separator)
...
self.rowID = Widget_Button(shapeID, Value = 'Row', /Checked_Menu)
self.columnID = Widget_Button(shapeID, Value = 'Column', /Checked_Menu)
self.pointID = Widget_Button(shapeID, Value = 'Point', /Checked_Menu, /Menu)
self.plusPointID = Widget_Button(self.pointID, Value = 'Plus Point', /Checked_Menu)
self.circlePointID = Widget_Button(self.pointID, Value = 'Circle Point', /Checked_Menu)
self.xPointID = Widget_Button(self.pointID, Value = 'X Point', /Checked_Menu)
;self.projectionID = Widget_Button(shapeID, Value = 'Projection', /Checked_Menu, /Separator)
...
self.fractionalID = Widget_Button(propertiesID, Value = 'Fractional', UValue = 'Fractional', /Checked_Menu, /Separator)
self.wholeID = Widget_Button(propertiesID, Value = 'Whole', UValue = 'Whole', /Checked_Menu)
IF self.regions_fractional THEN Widget_Control, self.fractionalID, Set_Button = 1 ELSE Widget_Control, self.wholeID, Set_Button = 1
...
smoothparamsID = Widget_Button(self.analysisID, Value = 'Smooth Parameters')
...
; Define some labels
IF self.view_info THEN BEGIN
   self.infoBaseID = Widget_Base(self.tlbID, Column = 2)
   labelsID = Widget_Base(self.infoBaseID, Column=1, /Frame)

   IF Obj_Valid(self.image) THEN self.image->GetProperty, filename = filename ELSE filename = ' '
   supertopID = Widget_Base(labelsID, Row = 1)
   filenameLabelID = Widget_Label(supertopID, Value = 'File', XSize = 58, /align_left)
   self.filenameLabelID = Widget_Label(supertopID, Value = filename, XSize = 250, frame=1, /align_left)

   toplabelsID = Widget_Base(labelsID, Row = 1)
   label = Widget_Label(toplabelsid, Value = 'Image', /align_left, XSize = 40)
   self.xlabelID = Widget_Label(toplabelsid, Value = ' X', /align_left, XSize = 15)
   self.xvalueID = Widget_Label(toplabelsid, Value = ' ', XSize = 80, frame=1, /align_right)
   self.ylabelID = Widget_Label(toplabelsid, Value = ' Y', /align_left, XSize = 15)
   self.yvalueID = Widget_Label(toplabelsid, Value = ' ', XSize = 80, frame=1, /align_right)
   valuelabelID = Widget_Label(toplabelsid, Value = ' Value', /align_left)
   self.valueID = Widget_Label(toplabelsid, Value = ' ', XSize = 80, frame=1, /align_right)

   bottomlabelsID = Widget_Base(labelsID, Row = 1)
   label = Widget_Label(bottomlabelsID,  Value = 'WCS  ', /align_left, XSize = 40)
   self.longlabelID = Widget_Label(bottomlabelsID, Value = ' l', /align_left, XSize = 15)
   self.longvalueID = Widget_Label(bottomlabelsID, Value = ' ', XSize = 80, frame=1, /align_right)
   self.latlabelID = Widget_Label(bottomlabelsID, Value = ' b', /align_left, XSize = 15)
   self.latvalueID = Widget_Label(bottomlabelsID, Value = ' ', XSize = 80, frame=1, /align_right)
...
ENDIF ELSE self.infoBaseID = -1

; Zoom draw windows
IF self.view_zoom THEN BEGIN
   minidrawwindowID = Widget_Base(self.infoBaseID, /Base_Align_Right, /Align_Right)
   self.zoom_DrawID = Widget_Draw(minidrawwindowID, XSize=85, YSize=85, /Align_Right)
ENDIF ELSE self.zoom_DrawID = -1
...
IF self.view_sliders THEN BEGIN
   self.slidersBaseID = Widget_Base(self.tlbID, /column)
   self.brightnessID = Widget_Slider(self.slidersBaseID, /suppress_value, xsize=self.xsize, /Drag, Minimum=0, Maximum=100, $
                                     UValue='brightness_slider', Value=50)
   self.contrastID = Widget_Slider(self.slidersBaseID, /suppress_value, xsize=self.xsize, /Drag, Minimum=0, Maximum=100, $
                                   UValue='contrast_slider', Value=50)
ENDIF ELSE self.slidersBaseID = -1
...
Widget_Control, self.tlbID, Base_Set_Title = 'Kang V1.3';self.windowTitle
...
; Bring up widgets
;IF self.show_plot_params_widget THEN self->plot_params_widget
;IF self.show_file_dialog THEN self->file_dialog_widget
...
;
;-
PRO Kang::View, info = info, menubar = menubar, sliders = sliders

IF Arg_Present(info) THEN self.view_info = info
;IF Arg_Present(menubar) THEN self.view_menubar = menubar
IF Arg_Present(sliders) THEN self.view_sliders = sliders

; Destroy, create components
IF ~self.view_info and self.infoBaseID NE -1 THEN Widget_Control, self.infoBaseID, /Destroy
IF ~self.view_info and self.infoBaseID NE -1 THEN Widget_Control, self.infoBaseID, /Destroy
;Widget_Control, self.menubarID, Map = self.view_menubar
;Widget_Control, self.brightnessID, Map = self.view_sliders
;Widget_Control, self.contrastID, Map = self.view_sliders

END; Kang::View---------------------------------------------------


;+
; Brings up DAOPhot widget
;
; :Private:
;-
...
IF XRegistered('Kang DAOPhot '+StrTrim(self.tlbID, 2)) THEN BEGIN
    ...
    RETURN
ENDIF
self.DAOPhot_WidgetID = Widget_Base(MBar = menubarID, Title = 'Kang DAOPhot', $
                                    UValue = self, Column = 2, tlb_frame_attr=1)

; Define the Save As pull-down menu.
FileID = Widget_Button(menubarID, Value='File', /Menu)
exportID = Widget_Button(fileID, Value = 'Export to file')
...
threshold_sliderID = Widget_Slider(sliderBase, min = 1, max = 21, xsize = 60)
...
aperBase = Widget_Base(CommandsBase, /Frame, /Column)
aperID = Widget_Button(aperBase, Value = ' Find Fluxes (Aper) ')

TableBase = Widget_Base(self.DAOPhot_WidgetID, column = 1)
tableID = Widget_Table(tableBase, column_labels = ['X Pos', 'Y Pos', 'Flux', 'Sharpness', 'Roundness'], $ 
                       Y_Scroll_Size = 21, /Scroll, xsize = 5, column_Widths = 80, background_color = [250,250,250], $
                      /Context)
infobarID = Widget_Label(tablebase, xsize = 500)
...
PRO Kang::Write_DAOPhot, filename

IF N_Elements(filename) EQ 0 THEN filename = Dialog_pickfile(filter = '*', /Write, Get_Path=thispath, Path=self.lastpath, $
                                                        dialog_parent = self.tlbID)

; Get info
self.image->GetProperty, filename = image_filename, stars = stars, skyval = skyval, skysig = skysig, fwhm = fwhm, $
                         ps_threshold = ps_threshold, sharplim = sharplim, roundlim = roundlim, err_fwhm = err_fwhm

; Convert to long/lat
longlat = self.image->Convert_coord(stars.x, stars.y, old_coord = 'image', new_coord = 'Galactic')

; Write to file
wh = where(stars.good)
;writecol, filename, ((stars.x)[stars.indices])[wh_good], ((stars.y)[stars.indices])[wh_good], ((stars.flux)[stars.indices])[wh_good], $
;          ((stars.sharpness)[stars.indices])[wh_good], ((stars.roundness)[stars.indices])[wh_good], $
;          Header = ['X', 'Y', 'Flux', 'Sharpness', 'Roundness'], $
;          Comments = '# ' + [Systime(), image_filename, 'FWHM: ' + StrTrim(fwhm, 2), 'Threshold: ' + StrTrim(ps_threshold, 2)]
writecol, filename, longlat[0, wh], longlat[1, wh], stars.x[wh], stars.y[wh], stars.flux[wh], stars.sharpness[wh], stars.roundness[wh], $
          Header = ['l', 'b', 'X', 'Y', 'Flux', 'Sharpness', 'Roundness'], $
          Comments = '# ' + [Systime(), image_filename, 'FWHM: ' + StrTrim(fwhm, 2), 'Threshold: ' + StrTrim(ps_threshold, 2)]

END; Kang::Write_DAOPhot--------------------------------------------------------
...
; Filter
Widget_Control, uval.sharp_minID, Set_Value = StrTrim(String(sharplim[0], format = '(f10.2)'), 2)
Widget_Control, uval.sharp_maxID, Set_Value = StrTrim(String(sharplim[1], format = '(f10.2)'), 2)
Widget_Control, uval.round_minID, Set_Value = StrTrim(String(roundlim[0], format = '(f10.2)'), 2)
Widget_Control, uval.round_maxID, Set_Value = StrTrim(String(roundlim[1], format = '(f10.2)'), 2)

; Table
IF size(stars, /tname) EQ 'STRUCT' THEN BEGIN
; It takes a while to update this, so let's only do it if it's different
    newtable = transpose([[([stars.x])[stars.indices]], [([stars.y])[stars.indices]], $
                          [([stars.flux])[stars.indices]], [([stars.sharpness])[stars.indices]], $
                          [([stars.roundness])[stars.indices]]])
    Widget_Control, uval.tableID, Get_Value = oldTable
    IF total(newtable EQ oldtable) NE N_Elements(oldtable) THEN BEGIN
        Widget_Control, uval.tableID, Set_Value = newTable, $
                        table_ysize = stars.n_stars, table_xsize = 5, column_Widths = 80, alignment = 2
    ENDIF
ENDIF

; Turn bad rows red - do sorting here though
IF size(stars, /tname) EQ 'STRUCT' THEN BEGIN
   bad = (stars.badsharp[stars.indices] + stars.badround[stars.indices] + stars.badcntrd[stars.indices]) < 1
   bgcolors = bytarr(3, stars.n_stars*5)
   bgcolors[*] = 255

   IF total(bad) NE 0 THEN BEGIN
      bgcolors[0, where(bad, count)*5] = 200
      bgcolors[0, where(bad, count)*5+1] = 200
      bgcolors[0, where(bad, count)*5+2] = 200
      bgcolors[0, where(bad, count)*5+3] = 200
      bgcolors[0, where(bad, count)*5+4] = 200
      bgcolors[1:2, where(bad, count)*5] = 75
      bgcolors[1:2, where(bad, count)*5+1] = 75
      bgcolors[1:2, where(bad, count)*5+2] = 75
      bgcolors[1:2, where(bad, count)*5+3] = 75
      bgcolors[1:2, where(bad, count)*5+4] = 75
   ENDIF

   Widget_Control, uval.TableID, background_color = bgcolors
ENDIF

; Info bar
IF size(stars, /tname) EQ 'STRUCT' THEN BEGIN
   foo = where(stars.badsharp, n_badsharp)
   foo = where(stars.badround, n_badround)
   foo = where(stars.badcntrd, n_badcntrd)
   foo = where(bad, n_bad)
   n_good = N_Elements(stars.x) - n_bad

   Widget_Control, uval.infobarID, Set_Value = '# Found: ' + StrTrim(n_good, 2)  + $
                   ' [# Rejected: Total (' + StrTrim(n_bad, 2) + $
                   ') Sharpness (' + StrTrim(n_badsharp, 2) + $ 
                   ') Roundness (' + StrTrim(n_badround, 2) + $ 
                   ') Centroid (' + StrTrim(n_badcntrd, 2) + ')]'
ENDIF
  
END; Kang::Update_DAOPhot_Widget----------------------------------------
...
XManager, 'File Dialog '+StrTrim(self.tlbID, 2), tlb, /No_Block, Event_Handler = 'Kang_File_Dialog_Events', $
    Group_Leader=self.tlbID, Cleanup = 'Kang_File_Dialog_Events'
...
self.axisColorID = CW_Drawcolor(colorsBase, Color=image->GetProperty(/axiscname), LabelText='Axis Color: ', $
                                Title='Axis Color:', UValue ='Axis Color', Rows = 4, cnames = *self.colornames)
self.backColorID = CW_Drawcolor(colorsBase, Color=image->GetProperty(/backcname), LabelText='     Background Color: ', $
                                Title='Axis Color:', UValue = 'Background Color', Rows = 4, cnames =  *self.colornames)
self.tickColorID = CW_Drawcolor(colorsBase, Color=image->GetProperty(/tickcname), LabelText='       Tick Color: ', $
                                Title='Axis Color:', UValue = 'Tick Color', Rows = 4, cnames = *self.colornames)
...
region_DeleteID = Widget_Button(otherBase, Value=' Delete ', UValue='Delete')
region_HistogramID = Widget_Button(otherBase,  Value='Histogram',  UValue='Histogram')
region_ZoomID = Widget_Button(otherBase,  Value='  Zoom  ',  UValue='Zoom to Region')
region_MoveUpID = Widget_Button(otherBase, Value = ' Move Up ',  UValue='Move Up')
region_MoveDownID = Widget_Button(otherBase, Value = 'Move Down',  UValue='Move Down')
region_MoveTopID = Widget_Button(otherBase, Value = 'Move Top',  UValue='Move Top')
region_MoveBottomID = Widget_Button(otherBase, Value = 'Move Bottom',  UValue='Move Bottom')
...
self.region_ColorID = CW_Drawcolor(toppropBase, Color='black', LabelText='Color: ', Title='Axis Color:', $
                                   UValue = 'Region Color', Rows = 4, CNames = *self.colornames)
...
XManager, 'Kang_Plot_Params_Widget ' +StrTrim(self.tlbID, 2), tlb, /No_Block, Group_Leader=self.tlbID, $
   cleanup='Kang_Plot_Params_Events'
...
; If nothing is there, update with ' ', otherwise use real values
IF n_selected NE 0 AND n_regions NE 0 THEN BEGIN
...
; UnMap current base
;    IF Widget_Info(self.current_regionBase, /realized) AND regionType[n_selected-1] NE self.plot_params_regionType THEN $
...
CASE regiontype OF
    ...
    'Composite': BEGIN
; Set list Value
        regiongroup_obj = self.regions->Get(count = count)
        regiongroup_obj[count-1]->GetProperty, alltext = alltext
    ...
    'Projection': BEGIN
        Widget_Control, uval.projection_p1_xID, Set_Value = params[0]
        Widget_Control, uval.projection_p1_yID, Set_Value = params[1]
        Widget_Control, uval.projection_p2_xID, Set_Value = params[2]
        Widget_Control, uval.projection_p2_yID, Set_Value = params[3]
;        Widget_Control, uval.projection_p2_yID, Set_Value = params[3]
    ENDCASE
...
ELSE image->GetProperty, astr = astr
...
print, pix_newlimits[4], pix_newlimits[5]
...
; Get spectra parameters    
    ...
    lineColorID = CW_Drawcolor(toprowBase, Color=lineCName, LabelText='Line Color: ', Title='Line Color:', $
                               UValue = 'Line Color', Rows = 4, CNames = *self.colornames)
    ...
    backColorID = CW_Drawcolor(toprowBase, Color=backCName, LabelText='BG Color: ', $
                               Title='BG Color:', UValue = 'BG Color', Rows = 4, CNames = *self.colornames)
...
self.profile_WidgetID = Widget_Base(Column=1, /Base_Align_Center, Group_Leader = self.tlbID, Title = 'Fit Profile')
...
; Menubar and events
    'WIDGET_BUTTON': BEGIN
        ...
        CASE buttonName OF
            ...
            'Save Object Reference': self->Save_ObjRef
            ...
            'Min/Max': self->Scale, percent = 100
            '99.99%': self->Scale, percent = 99.99
            '99.9%': self->Scale, percent = 99.9
            '99.7%': self->Scale, percent = 99.7
            '99.5%': self->Scale, percent = 99.5
            '99%': self->Scale, percent = 99
            '98%': self->Scale, percent = 98
            '97%': self->Scale, percent = 97
            '95%': self->Scale, percent = 95
            'Scale to Image': stop
            ...
            'Projection': self->Global_Region_Property, RegionType = 'Projection'
            'Row': self->Global_Region_Property, RegionType = 'Row'
            'Column': self->Global_Region_Property, RegionType = 'Column'
            'Point': self->Global_Region_Property, RegionType = 'Point'

            'Circle Point': self->Global_Region_Property, PointType = 'Circle'
            'Plus Point': self->Global_Region_Property, PointType = 'Plus'
            'X Point': self->Global_Region_Property, PointType = 'X'
            ...
            'Fractional': self->Global_Region_Property, /Fractional
            'Whole': self->Global_Region_Property, /Whole
            ...
            '3D': self->threeD
            'Smooth': self->SetProperty, smooth = event.click
            'Smooth Parameters': self->Smooth_Widget
...
; If user deleted object from command line, remove GUI display
IF ~Obj_Valid(self) THEN Widget_Control, self.tlbID, /Destroy
...
; Calculate the size of one pixel on the monitor in data units
    IF event.key GE 5 AND event.key LE 8 THEN BEGIN
...
; Arrow  keys move the regions around
        'Left': IF event.press THEN self->Move_Region, [-pixelsize, 0], coordinate = 'Image'
        'Right': IF event.press THEN self->Move_Region, [pixelsize, 0], coordinate = 'Image'
        'Up': IF event.press THEN self->Move_Region, [0, pixelsize], coordinate = 'Image'
        'Down': IF event.press THEN self->Move_Region, [0, -pixelsize], coordinate = 'Image'
        ELSE:
    ENDCASE
ENDELSE
...
; Single click - Turn on the flags - regions selected on up click
       IF event.clicks LE 1 THEN BEGIN
...
; Double-clicks - select on second down click
          self.movingRegion = 1B
          self.drawingRegion = 0B
          self.selectingRegion = 0B
          hits = where(self.regions->Contains_Points(self.current_pix[0], self.current_pix[1]) EQ 1)
          hits = hits[N_Elements(hits)-1] ; Only take last one
          
; Clear selected regions if click is outside all regions
          IF hits[0] EQ -1 THEN self.regions->Select, /None ELSE BEGIN

; If CTRL is held down, add to selection.  Otherwise, selection is in 'hits'
             IF self.control THEN BEGIN
                self.regions->GetProperty, selected_regions = selected_regions
                IF selected_regions[0] EQ -1 THEN selected_regions = hits ELSE BEGIN
                   overlap = setIntersection(hits, selected_regions)
                   nonOverlap = setDifference(hits, selected_regions)
...
; No control being held down
             ENDIF ELSE self.regions->Select, hits
          ENDELSE
       ENDELSE

; Store mouse button downs
...
; Just erase the box if the user just clicked up and down
            IF ((self.initial_pix[0] EQ self.current_pix[0]) AND (self.initial_pix[1] EQ self.current_pix[1]) $
                ...
                AND (StrUpCase(self.regionType) NE 'TEXT') AND (StrUpCase(self.regionType) NE 'POINT') $
                AND (StrUpCase(self.regionType) NE 'ROW') AND (StrUpCase(self.regionType) NE 'COLUMN')) THEN BEGIN
                self->Copy_Display
...
; Make some strings for the regions
            ...
            self.image->GetProperty, current_coords = coordinates
; I have no idea where the factor of 1.25 comes from!
            IF StrUpCase(coordinates) EQ 'J2000' OR StrUpCase(coordinates) EQ 'FK5' OR $
               ...
               distance_data = ((self.final_data[0]-self.initial_data[0])^2.+((self.final_data[1]-self.initial_data[1])*1.25)^2.)^.5
            ENDIF ELSE distance_data = ((self.final_data[0]-self.initial_data[0])^2.+(self.final_data[1]-self.initial_data[1])^2.)^.5
...
; Compute major and minor axis
                ...
                'Row': self->Make_Region, 'Row', self.final_data[1]

                'Column': self->Make_Region, 'Column', self.final_data[0]

                'Point': self->Make_Region, 'Point', self.final_data
...
;                END
                ...
                'Projection': self->Make_Region, 'Projection', [self.initial_data[0], self.initial_data[1], self.final_data[0], self.final_data[1], 0]
...
; I'm going to piggyback on the plot_params event handler here
            ...
            fractionalID = Widget_Button(regionButton, Value = 'Fractional', /Checked_Menu, /Separator)
            wholeID = Widget_Button(regionButton, Value = 'Whole', /Checked_Menu)
...
; Set buttons
            self.regions->GetProperty, interior = exclude, source = source, polygon = polygon, thick = width, $
              linestyle = linestyle, position = max(where(hits EQ 1)), weighted = weighted
            ...
            IF weighted THEN Widget_Control, fractionalID, Set_Button = 1 ELSE Widget_Control, wholeID, Set_Button = 1
...
self.old_current_device = self.current_device
self.current_device = [event.x, event.y]
...
IF self.current_pix[0] GT pix_limits[1] OR self.current_pix[1] GT pix_limits[3] OR $
   self.current_pix[0] LT pix_limits[0] OR self.current_pix[1] LT pix_limits[2] THEN BEGIN
   Widget_Control, self.xvalueID, Set_Value = ' '
   Widget_Control, self.yvalueID, Set_Value = ' '
   Widget_Control, self.longvalueID, Set_Value = ' '
   Widget_Control, self.latvalueID, Set_Value = ' '
   Widget_Control, self.valueID, Set_Value = ' '
ENDIF ELSE BEGIN
   value = self.image->Integrate(self.current_pix[0], self.current_pix[1])
...
; Update the values shown on top ------------------------------------------------------------
   IF self.xvalueID NE 0 THEN BEGIN

; Pixels
      Widget_Control, self.xvalueID, Set_Value = String(self.current_pix[0], format = '(f10.4)')
      Widget_Control, self.yvalueID, Set_Value = String(self.current_pix[1], format = '(f10.4)')

; Astronomical Coords
      IF sexadecimal THEN data = [kang_sexadecimal(self.current_data[0]), kang_sexadecimal(self.current_data[1])] ELSE $ 
         data = String(self.current_data, format = '(F10.4)')
      Widget_Control, self.longvalueID, Set_Value = data[0]
      Widget_Control, self.latvalueID, Set_Value = data[1]

; Set Value
      Widget_Control, self.valueID, Set_Value = String(Value, format = '(f10.4)')
   ENDIF
ENDELSE

; Region Moving --------------------------------------------------------------------------------
IF self.movingRegion THEN BEGIN
...
; Move region
   translation = (self.current_device - self.old_current_device) * pixelsize
   self->Move_Region, translation, coord = 'Image'
ENDIF
...
IF self.drawingRegion THEN BEGIN
    ...
    CASE self.regionType OF
        ...
        'Row': plots, [pix_newlimits[0]-0.5, pix_newlimits[1]+0.5], round([self.current_pix[1], self.current_pix[1]]), $
                      color = self.drawingcolor, thick = self.regions_width, linestyle = self.regions_linestyle

        'Column': plots, round([self.current_pix[0], self.current_pix[0]]), [pix_newlimits[2]-0.5, pix_newlimits[3]+0.5], $
                         color = self.drawingcolor, thick = self.regions_width, linestyle = self.regions_linestyle

        'Point': plots, [self.current_pix[0], self.current_pix[1]], psym = 1
        ...
        'Projection': plots, [self.initial_pix[0], self.current_pix[0]], [self.initial_pix[1], self.current_pix[1]], $
                       color = self.drawingcolor, thick = self.regions_width, linestyle = self.regions_linestyle
...
IF !D.Name EQ 'X' OR !D.Name EQ 'WIN' AND self.xvalueID AND self.view_zoom THEN BEGIN
...
colors = Obj_New('Kang_Colors', colornames=*self.colornames, colorindex = 3)
...
maskColor = colors->ColorIndex('White')
...
oimage->SetProperty, axisColor = axisColor, AxisCname = 'Black', backColor = backColor, backCname = 'White', $
  maskColor = maskColor, maskCName = 'Green', tickColor = tickColor, tickCname = 'Black', NColors = NColors
...
;+
; Saves object reference as a variable in case user forgot at startup
;
; :Private:
;-
PRO Kang::Save_ObjRef, varname

IF N_Elements(varname) EQ 0 THEN varname = Kang_Textbox(Title='Object Reference Name:', Group_Leader=self.tlbID, $
                                           Label=['Object Reference Name:'], Cancel=cancelled, XSize=100, Value='')

IF cancelled THEN RETURN

; Store
(SCOPE_VARFETCH(varname, /ENTER, LEVEL=1)) = self

END; Kang::Save_ObjRef-----------------------------------------------------------------
...
; Menubar and events
    'WIDGET_BUTTON': BEGIN
        Widget_Control, event.ID, Get_Value = buttonName
        CASE StrTrim(buttonName,2) OF
           'Export to file': uval.self->Write_DAOPhot
           'Find New Sky Level': BEGIN
              Widget_Control, uval.meanmodeID, Get_Value = meanmode
              uval.self->Sky, meanback = ~meanmode
           END
           ...
           END
           'Filter Results': BEGIN
              Widget_Control, uval.sharp_minID, Get_Value = sharp_min
              Widget_Control, uval.sharp_maxID, Get_Value = sharp_max
              Widget_Control, uval.round_minID, Get_Value = round_min
              Widget_Control, uval.round_maxID, Get_Value = round_max
              uval.self->Filter, sharplim = [sharp_min, sharp_max], roundlim = [round_min, round_max]
           END
           'Find Fluxes (Aper)': uval.self->Aper
        ENDCASE
...
; Right click events from the widget_table
; Row = -1 is the top row of column headings
        IF event.row EQ -1 THEN BEGIN
            CASE event.col OF
                -1:
                0: uval.self->Sort_Stars, /x
                1: uval.self->Sort_Stars, /y
                2: uval.self->Sort_Stars, /flux
                3: uval.self->Sort_Stars, /sharp
                4: uval.self->Sort_Stars, /round
            ENDCASE
        ENDIF
        IF event.col EQ -1 AND event.row NE -1 THEN BEGIN
           Widget_Control, uval.tableID, Get_Value = data
           uval.self->Zoom, data[0, event.row], data[1, event.row], /pix
        ENDIF
    END
    ELSE:
ENDCASE
...
; These are kill events
IF size(event, /tname) NE 'STRUCT' THEN BEGIN
    Widget_Control, event, Get_UValue=self
;    self->SetProperty, show_file_dialog_widget = 0
    RETURN
ENDIF
...
IF thisEvent EQ 'WIDGET_BUTTON' THEN BEGIN
   ...
   IF buttonName EQ '  Apply  ' THEN BEGIN

; Get parameters out
       Widget_Control, uval.smoothTypeID, Get_Value = type
       smooth_type = (['Boxcar', 'Median', 'Gaussian'])[type]
       Widget_Control, uval.radius_sliderID, Get_Value = smooth_radius

; Smooth
       Widget_Control, /Hourglass
       uval.self->Smooth, smooth_radius, type = smooth_type
       Widget_Control, Hourglass=0
   ENDIF
ENDIF

END; Kang_Smooth_Events---------------------------------------
...
CASE buttonValue OF
    ...
    'Fractional': self->Region_Property, /Fractional, position = self.whichInside
    'Whole': self->Region_Property, /Whole, position = self.whichInside
...
IF size(event, /tname) NE 'STRUCT' THEN BEGIN
    Widget_Control, event, Get_UValue=self
;    self->SetProperty, show_plot_params_widget = 0
    RETURN
ENDIF
...
; User clicked inside the list widget. Update selected regions tag
    ...
    'Write': self->Write_RegionW
    'Histogram': self->Histogram_Region
    'Zoom to Region': self->Zoom_Region
    'Move Up': self->Order_Region, /Backward
    'Move Down': self->Order_Region, /Forward
    'Move Top': self->Order_Region, /ToBack
    'Move Bottom': self->Order_Region, /ToFront
...
; Apphot things
    'Recompute': foo = self->Apphot(/recompute)
    'Source Droplist': self->Update_Apphot_Widget, event.index
    ELSE:
ENDCASE
...
CASE UValue OF
    ...
    'Projection': BEGIN
        Widget_Control, IDs.projection_p1_xID, Get_Value = p1_x 
        Widget_Control, IDs.projection_p1_yID, Get_Value = p1_y
        Widget_Control, IDs.projection_p2_xID, Get_Value = p2_x
        Widget_Control, IDs.projection_p2_yID, Get_Value = p2_y

        params = [p1_x, p1_y, p2_x, p2_y]
        self->Make_Region, 'Projection', params, data = data, /noobject
    END
...
; These are the velocity buttons along the bottom of the widget
    'Button': BEGIN
        Widget_Control, event.id, Get_Value=buttonValue
        self->Velocity, deltavel = buttonValue, whichImage = whichVelocity
        ;velchange         self->Velocity, deltavel = buttonValue * 1000., whichImage = whichVelocity
    END
ENDCASE
...
;+
; Allows the user to load analysis commands from file
;
; :Params:
;   filename : in, required, type = string
;
;-
PRO Kang::Analysis, filename, _extra = extra

IF N_Elements(filename) EQ 0 THEN BEGIN
   print, 'ERROR: Filename or lines must be specified.'
   RETURN
ENDIF

; Call procedure
Call_Procedure, 'test', obj = self, _extra = extra
...
;      Gaussian, Lorentzian, Moffat, or Ellipse.  Defaults to
;      Gaussian.
;
...
self.image->GetProperty, pix_limits = pix_limits
xrange = [(pix_limits[1] < min(floor(xrange))) > 0, (0 > max(ceil(xrange))) < pix_limits[1]]
yrange = [(pix_limits[3] < min(floor(yrange))) > 0, (0 > max(ceil(yrange))) < pix_limits[3]]
...
; Things did not work out
    IF N_Elements(zfit) EQ 1 THEN RETURN
    
    self.image->GetProperty, fitParams = fitParams
...
; Get values out of dialog
            ...
            self->Fit_Profile, fitType, circular = circular, tilt = tilt
        END
        
        'Create Region': BEGIN
            self->GetProperty, fitParams = fitParams
; Create ellipse region
            self->Make_Region, 'Ellipse', [fitParams[4], fitParams[5], fitParams[2], fitParams[3], fitParams[6]]
        END
...
; Calculate the velocity for the x-axis in the plot
                ...
                vel_array = scale_vector(findgen(n_vel), limits[4], limits[5])
;                vel_array = scale_vector(findgen(n_vel), limits[4]*1000., limits[5]*1000.)/1000. or somehting
        ;velchange
...
; Find velocity limits
                IF StrTrim(lowrange, 2) EQ 'NULLVALUE' THEN lowrange = limits[4]
                IF StrTrim(highrange, 2) EQ 'NULLVALUE' THEN highrange = limits[5]
                pix_lowrange = Round((lowrange - astr.crval[2])/astr.cdelt[2] + astr.crpix[2] - 1) > 0
                pix_highrange = Round((highrange - astr.crval[2])/astr.cdelt[2] + astr.crpix[2] - 1) < pix_limits[5]
...
;                pix_lowrange = Round((lowrange*1000. - astr.crval[2])/astr.cdelt[2] + astr.crpix[2] - 1) > 0
;                pix_highrange = Round((highrange*1000. - astr.crval[2])/astr.cdelt[2] + astr.crpix[2] - 1) < pix_limits[5]
; velchange
...
; Fit Gaussians
                ...
                gauss_x = scale_vector(findgen(nchan), lowrange, highrange)
...
; Draw the display
IF self.mask THEN mask = self.regions->ComputeMask(/selected)
self.image->Draw, noimage = noimage, noaxis = noaxis, nobar = nobar, position = position, scaling = scaling, mask = mask
...
plot, [0], XRange = [pix_newlimits[0]-0.5, pix_newlimits[1]+0.5], YRange = [pix_newlimits[2]-0.5, pix_newlimits[3]+0.5], $
      XStyle=5, YStyle=5, /NoData, /NoErase, position = position
print, [pix_newlimits[0]-0.5, pix_newlimits[1]+0.5]
self.regions->Draw
...
;  GUI=gui, $                    ;Boolean for whether to bring up GUI
  ...
  ViewMenubar=viewmenubar, $    ; View menu bar at top of display
  ViewSliders=viewsliders, $    ; View sliders at bottom of display
  ViewZoom=viewzoom, $          ; View small zoom window
...
;device, retain = 2
self.colornames = Ptr_New(['white','pink','red','orange red','orange','gold','yellow',$
                           'green yellow','green','dark green','cyan','sky blue','steel blue',$
                           'blue','navy','violet','magenta','purple','maroon','brown','tan','beige',$
                           'light gray','medium gray','gray','dark gray','charcoal','black'])
...
; Viewing properties
IF N_Elements(viewinfo) NE 0 THEN self.view_info = viewinfo ELSE self.view_info = 1
IF N_Elements(viewmenubar) NE 0 THEN self.view_menubar = viewmenubar ELSE self.view_menubar = 1
IF N_Elements(viewsliders) NE 0 THEN self.view_sliders = viewsliders ELSE self.view_sliders = 0 ; By default let's have them off
IF N_Elements(viewzoom) NE 0 THEN self.view_zoom = viewzoom ELSE self.view_zoom = 0 ; By default let's have them off
...
; If an image was successfully loaded, set user defined parameters
    IF good_image THEN BEGIN
        ...
        IF n_Velocity GT i THEN BEGIN
            IF n_Deltavel GT i THEN deltavel = deltavel[i]
            self.image->SetVelocity, velocity[i], deltavel = deltavel
...
self.show_plot_params_widget = 1B
self.show_file_dialog = 1B
self.show_plot_windows = 1B
;self.show_velocity_widget = 1B
...
; Main widget display
          menubarID:0L, $       ; ID for menubar
          infoBaseID:0L, $      ; ID for info about position, zoom window, etc.
          ...
          filenamelabelID:0L, $ ; The filename
          ...
          longvalueID:0L, $
          latvalueID:0L, $
          slidersBaseID:0L, $   ; The contrast, brightness sliders at the bottom of display
...
; Viewing options         
          view_info:0B, $       ; View info panel at top?
          view_menubar:0B, $    ; View menubar at top?
          view_sliders:0B, $    ; View brightness and contrast sliders at bottom?
          view_zoom:0B, $       ; View zoom window?
...
;---------------Flags for bringing up widgets------------------------
          show_plot_params_widget:0B, $
          show_file_dialog:0B, $
          show_plot_windows:0B, $
;          show_velocity_widget:0B, $
...
;          pix_wid3:0L, $         ; The pixmap identifier where annotations and images are combined
          ...
          current_data:[0., 0.], $       ; Current mouse position in data coords ([x, y])
          current_device:[0L, 0L], $     ; Current mouse position in device coords ([x, y])
          old_current_device:[0L, 0L], $ ; Previous current mouse position in device coords ([x, y])
          ...
          drawingRegion:0B, $            ; Boolean for whether user is currently drawing a region
          movingRegion:0B, $             ; Boolean for whether user is currently moving a region
...
; Region variables
          ...
          regions_source:0B, $           ; Boolean for whether regions being drawn are source or background
          regions_fractional:0B, $       ; Boolean for whther regions calculated using fractional pixels
          ...
          nostats:0, $                   ; Boolean for whether to compute region stats
          ...
          pointType:'', $                ; Type of point to be draw if point region selected
...
; Analysis
          analysis:Ptr_New(), $          ; Analysis commands
          ...
          smoothID:0L, $                 ; ID of menu button to smooth image
          velfieldID:0L, $               ; ID for the velocity field dialog
          DAOPhot_WidgetID:0L, $         ; ID for the DAOPhot dialog
          apphot_WidgetID:0L, $          ; ID for apphot widget
          apphot:Ptr_New(), $            ; apphot structure.  Needs to be pointer because all can be arrays
...
; Colors
          ...
          colornames:Ptr_New(), $ ; Array of color names
