...
PRO Kang_Region_Container::Combine, nostats = nostats
...
; Kludge for now since does not retain regiongroups when dissolved
FOR i = 0, count-1 DO BEGIN
    IF objs[i]->GetProperty(/composite) THEN objs[i] = objs[i]->Get(position = 0)
ENDFOR

ogroup = Obj_New('Kang_RegionGroup', objs, regiontype = 'Composite', nostats = nostats, image_obj = self.image_obj)
...
IF Arg_Present(n_selected) THEN n_selected = self.n_selected
...
IF N_Elements(position) EQ 0 THEN BEGIN
   ...
   IF position[0] EQ -1 THEN IF self.n_regions GT 0 THEN position = lindgen(self.n_regions)
ENDIF
...
; THIS DOES NOT WORK!!!!
    ...
    IF Arg_Present(stats) THEN BEGIN
       IF n GE 1 THEN BEGIN
          stats = {filename:'', $
                   ...
                   max:FltArr(n), $
                   min:FltArr(n), $
                   centroid_min:FltArr(2, n), $
                   centroid_max:FltArr(2, n), $
                   mean:FltArr(n), $
                   median:FltArr(n), $
                   stddev:FltArr(n), $
                   total:FltArr(n)}
          
          self.image_obj->GetProperty, filename=filename0
          stats.filename = filename0
          FOR i = 0, n-1 DO BEGIN
             objs[i]->GetProperty, name = name0
             stats.name[i] = name0
             objs[i]->GetProperty, regiontext = regiontext0
             stats.regiontext[i] = regiontext0
             objs[i]->GetProperty, stats = stats0
             stats.coordinate = stats0.coordinate
             stats.sexadecimal = stats0.sexadecimal
             stats.unit = stats0.unit
             stats.area[i] = stats0.area
             stats.centroid[*, i] = stats0.centroid
             stats.perimeter[i] = stats0.perimeter
             stats.npix[i] = stats0.npix
             stats.n_nan[i] = stats0.n_nan
             stats.max[i] = stats0.max
             stats.min[i] = stats0.min
             stats.centroid_min[*, i] = stats0.centroid_min
             stats.centroid_max[*, i] = stats0.centroid_max
             stats.mean[i] = stats0.mean
             stats.median[i] = stats0.median
    ...
    IF Arg_Present(output_text) THEN BEGIN
       output_text = StrArr(n)
       FOR i = 0, n-1 DO output_text[i] = objs[i]->GetProperty(/output_text, image_obj = self.image_obj)
    ENDIF
    ...
    ENDIF
    IF Arg_Present(weighted) THEN BEGIN
       weighted = LonArr(n)
       FOR i = 0, n-1 DO weighted[i] = objs[i]->GetProperty(/weighted)
    ENDIF
...
FUNCTION Kang_Region_Container::Get_Stats, position
...
IF N_Elements(position) NE 0 THEN BEGIN
   ...
   IF size(position, /tname) NE 'STRING' THEN self->Select, position ELSE self->Select, name = position
ENDIF ELSE BEGIN
   IF self.n_selected EQ [-1] THEN BEGIN
      selected = [-1]
      self->Select, /all
   ENDIF
ENDELSE
...
; Select original regions
IF N_Elements(selected) NE 0 THEN self->select, selected

RETURN, stats
END; Kang_Region_Container::Get_Stats-----------------------------------------------


FUNCTION Kang_Region_Container::Histogram, position, bins = bins, reverse_indices = reverse_indices, _Extra = extra
...
; Compute histogram.  Do more than one region in the future
obj = self->Get(position = position[0])
histvals = obj->Histogram(image_obj = self.image_obj, bins = bins, reverse_indices = reverse_indices, _Extra = extra)

; Select original regions
IF N_Elements(selected) NE 0 THEN self->select, selected

RETURN, histvals
END; Kang_Region_Container::Histogram-----------------------------------------------
...
; angle
    ...
    IF N_Elements(weighted) NE 0 THEN obj->SetProperty, weighted = weighted, image_obj = self.image_obj
...
PRO Kang_Region_Container::Select, positions_in, Add = add, All = All, First = first, Include = include, $
  Exclude = exclude, Invert = invert, Last = last, Name = name, None = none, $
...
IF Keyword_Set(exclude) OR Keyword_Set(include) OR Keyword_Set(source) OR Keyword_Set(Background) OR N_Elements(name) NE 0 THEN BEGIN

; Get arrays
    self->GetProperty, position = lindgen(self.n_regions), interior = exclude_arr, background = background_arr, name = region_name
...
; These will create large arrays that need to be uniqueified
    ...
    IF n NE 0 THEN BEGIN
       selected = lonArr(n)
       FOR i = 0, n-1 DO IF name[i] NE '' THEN selected[i] = where(StrUpCase(name[i]) EQ StrUpCase(region_name)) ELSE selected[i] = -1
; Remove -1s
       wh = where(selected NE -1, count, complement = complement)
       IF count GT 0 THEN selected = selected[wh] ELSE selected = [-1]
       IF count NE n THEN print, 'The following regions were not found: ' + StrJoin(name[complement], ', ')
    ENDIF
ENDIF
...
; Update n_selected
foo = N_Elements(where(*self.selected_regions GT -1, n_selected))
...
; Locate composite regions.  Take both "#composite" and "composite" as in DS9
composite = StrMatch(StrCompress(regions, /Remove), 'Composite*', /Fold_Case) EQ 1 OR StrMatch(StrCompress(regions, /Remove), '#Composite*', /Fold_Case) EQ 1
wh_composite = where(composite EQ 1, n_composite)

; Remove comments
wh_not_comments = where(StrMatch(regions, '#*') EQ 0 OR composite EQ 1)
IF wh_not_comments[0] NE -1 THEN regions = regions[wh_not_comments] ELSE RETURN ; Ideally should keep these somewhere....
...
name = StrArr(n_regions)
pointType = StrArr(n_regions)
type = BytArr(n_regions)
...
source[*] = 1B
name[*] = ''
type[*] = 2B                    ; all polygons by default
...
wh = where(composite_array EQ 1, count)
IF count GT 0 THEN num_pos[wh] = StrPos(regions[wh], '||')
...
; Name
        wh_name = (where(StrMatch(other_params, 'name', /Fold_Case) EQ 1))[0]
        IF wh_name NE -1 THEN source[i] = 0
; Point Type
        wh_point = (where(StrMatch(other_params, 'point=*', /Fold_Case) EQ 1))[0]
        IF wh_point NE -1 THEN pointType[i] = StrTrim(StrMid(other_params[wh_point], 6), 2)
; Line or polygon
        wh_line = (where(StrMatch(other_params, 'type=Line', /Fold_Case) EQ 1))[0]
        IF wh_line NE -1 THEN type[i] = 1B
    ENDIF
...
; Find the indices
    ...
    CASE StrUpCase(region_Type[i]) OF
        ...
        'COLUMN': BEGIN
           self.image_obj->getproperty, limits = limits
           data = [[limits[0], reg[0]], [limits[1], reg[0]]]
; Convert to data, then back to pixels
           data = round(self.image_obj->Convert_Coord(data, new_coord = 'Image'))+[[-0.5, 0], [0.5, 0]]
...
; Find the x and y points
        ...
        'PROJECTION': BEGIN
            data = [[reg[0], reg[1]], [reg[2], reg[3]], reg[4]]
            type[i] = 1
        END
...
; Convert to data, then back to pixels
        ...
        '#COMPOSITE' OR 'COMPOSITE': BEGIN
           ...
           regiongroup_obj = Obj_New('Kang_RegionGroup', angle = angle, color = color_new, cname = color[i], $
                                     ...
                                     regiontype = 'Composite', text = text[i], name = name, source = source[i], $
                                     pointType = pointType[i], linestyle = 0, type = type[i], nostats = nostats, polygon = polygon)
           composite_array[i] = 3
         END
        ELSE: print, region_type[i]
    ENDCASE

; Convert coordinates if necessary
;    self.image_obj->GetProperty, current_coords = current_coords
;    IF coord_type[i] NE current_coords THEN data = self.image_obj->Convert_coord(data, old_coords = coord_type[i])
...
; Create object
        polygon = type[i] EQ 2
        region_obj = Obj_New('Kang_Region', data, params = reg, angle = angle, coordinate = coord_type[i], $
                             color = color_new, cname = color[i], thick=width[i], interior = exclude[i], $
                             image_obj = self.image_obj, regiontype = region_type[i], pointType = pointType[i], $
                             text = text[i], name = name, source = source[i], linestyle = 0, type = type[i], nostats = nostats, $
                             polygon = polygon)
    ENDIF
...
IF self.n_regions EQ 0 THEN BEGIN
    print, 'No regions!  Nothing to save, returning....'
    RETURN
ENDIF
...
; Get info
self->GetProperty, regiontype = regiontype, output_text = text, position = lindgen(self.n_regions)
...
; Threshold regions need to be converted to polygons
   IF objrefs[i]->GetProperty(/composite) EQ 1 THEN BEGIN

; Account for extra parameters at end of region string
      PrintF, lun, text[i]

; Get region text
        objrefs[i]->GetProperty, alltext = alltext, n_regions = n_composite_regions, image_obj = self.image_obj

; Print other parts of the composite but don't do the last one
        FOR j = 0, n_composite_regions-2 DO PrintF, lun, repstr(alltext[j], '#', '|| #')
        PrintF, lun, alltext[n_composite_regions-1]
    ENDIF ELSE PrintF, lun, text[i]
ENDFOR
free_lun, lun
...
PRO Kang_Region_Container::Order, region_to_move, destination = destination, tofront = tofront, toback = toback, $
  forward = forward, backward = backward

; Default to selected regions
IF N_Elements(regions_to_move) EQ 0 THEN regions_to_move = *self.selected_regions
regions_to_move = uniq(regions_to_move, sort(regions_to_move))

; Inputs must match
IF N_Elements(destination) GT 1 THEN IF N_Elements(destination) NE N_Elements(regions_to_move) THEN RETURN
destination = uniq(destination, sort(destination))

; Change destination based on keywords
IF Keyword_Set(tofront) THEN destination = 0
IF Keyword_Set(toback) THEN destination = self.n_regions-1
IF Keyword_Set(forward) THEN BEGIN
    IF max(regions_to_move) GT (self.n_regions-1) THEN RETURN

; Set order correctly
;    max = self.n_regions - 1
;    FOR i = N_Elements(destination)-1 DO BEGIN
    destination = (regions_to_move+1) < (self.n_regions-1)
ENDIF
IF Keyword_Set(backward) THEN BEGIN
    IF min(regions_to_move) EQ 0 THEN RETURN
    destination = regions_to_move-1
ENDIF
IF N_Elements(destination) EQ 0 THEN RETURN

; Move regions using IDL's container function
FOR i = 0, N_Elements(destination)-1 DO self.region->Move, regions_to_move[i], destination[i]

; Update selected_regions.  If trying to move many to the same
; location, this will select correctly 

; No problems
IF N_Elements(uniq(destination)) EQ N_Elements(regions_to_move) THEN *self.selected_regions = destination ELSE BEGIN
; user tried to push regions off the edge
    u = uniq(destination)
    *self.selected_regions = destination[u]
    regions_to_move[u] = -1
    *self.selected_regions = [*self.selected_regions, regions_to_move[where(regions_to_move NE -1)]]
    *self.selected_regions = (*self.selected_regions)[sort(*self.selected_regions)]
ENDELSE

END; Kang_Region_Container::Reorder-------------------------------------------------------
...
PRO Kang_Region_Container::Move, translation, coordinate = coordinate, position = position
...
objs = self->Get(position = position)
FOR i = 0, n_pos-1 DO objs[i]->Move, translation, coordinate = coordinate, image_obj = self.image_obj

END; Kang_Region_Container::Move-------------------------------------------------


FUNCTION Kang_Region_Container::ComputeMask, all = all, bitwise = bitwise, inverted = inverted, numbered = numbered, selected = selected, $
   position=position, weighted = weighted
; Computes a mask for the regions.  Default is to mask all regions.
; If bitwise is set, mask cannot be weighted

; Find dimensions, return full mask if no regions loaded
self.image_obj->GetProperty, dimensions = dimensions
IF self.n_regions EQ 0 THEN RETURN, bytarr(dimensions)
IF Keyword_Set(selected) AND (*self.selected_regions)[0] EQ -1 THEN RETURN, bytarr(dimensions)

; Create position array
n_pos = N_Elements(position)
IF n_pos EQ 0 THEN BEGIN
   IF (*self.selected_regions)[0] EQ -1 THEN all = 1
   IF Keyword_Set(all) THEN position = lindgen(self.n_regions) ELSE position = *self.selected_regions
ENDIF
n_pos = N_Elements(position)
print, n_pos

; Create mask
IF ~Keyword_Set(bitwise) AND ~Keyword_Set(numbered) THEN BEGIN
...
ENDIF
IF Keyword_Set(bitwise) AND ~Keyword_Set(numbered) Then BEGIN
   IF N_Elements(position) LE 14 THEN mask = lonarr(dimensions)
   IF N_Elements(position) LE 127 THEN mask = fltarr(dimensions)
   IF N_Elements(position) GT 127 THEN mask = dblarr(dimensions)
   FOR i = 0, n_pos-1 DO BEGIN
      obj = self->Get(position = position[i])
      mask = mask + obj->ComputeMask(dimensions = dimensions)/255B * 2d^(i+1)
   ENDFOR
ENDIF
; If bitwise and numbered are both set for some reason, use numbered
IF Keyword_Set(numbered) THEN BEGIN
   mask = lonarr(dimensions)
   FOR i = 0, n_pos-1 DO BEGIN
      obj = self->Get(position = position[i])
      mask += (obj->ComputeMask(dimensions = dimensions, weighted = weighted)/255B*(i+1))
   ENDFOR
ENDIF

IF Keyword_Set(inverted) THEN RETURN, ~(mask/255B) ELSE RETURN, mask/255B
...
; Draw source boxes solid, background boxes open
        IF source THEN psym = symcat(15) ELSE psym = symcat(6)
	CASE regiontype OF
	    'Text':
            'Point':
	    'Line': BEGIN
	              objs[i]->GetProperty, data = data
                      plots, data[0, *], data[1, *], psym = psym, color = color, noclip = 0
	    END
	    'Projection': BEGIN
	              objs[i]->GetProperty, data = data
                      plots, data[0, *], data[1, *], psym = psym, color = color, noclip = 0
	    END
	    ELSE: plots, [xrange[0], xrange[0], xrange[1], xrange[1]], [yrange[0], yrange[1], yrange[1], yrange[0]], psym = psym, $
...
struct = {KANG_REGION_CONTAINER, $
          ...
          n_selected:0L, $             ; Number of regions currently selected
