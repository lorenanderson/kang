...
PRO Kang_Spectrum::Gauss, n_gauss, xrange = xrange, noclick = noclick
...
IF ~Keyword_Set(noclick) THEN BEGIN
self->Draw
...
ENDIF ELSE BEGIN
    ...
    epix = 325
weightf=fltarr(epix-bpix+1)+1.0 ; weight 1.0 in mask region
max = max((*self.y)[bpix:epix], max_pix)
a = [max, max_pix, 5]
print, a
plot, (*self.x)[bpix:epix], (*self.y)[bpix:epix]
yfit = mpcurvefit((*self.x)[bpix:epix], (*self.y)[bpix:epix], weightf, a, fltarr(3), function_name='mgauss',$
                  chisq=chisq, itmax=500, /quiet)
ENDELSE
