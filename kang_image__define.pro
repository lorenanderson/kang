...
;	extension = where(StrUpCase(fcb.extname) EQ 'IMAGE') > 0
;        fits_close, fcb

; Read in image
;        image = ReadFits(filename, header, exten_no = extension[0], /Silent)
...
IF N_Elements(maskColor) NE 0 THEN self.maskColor=maskColor ELSE self.maskColor = 255B
IF N_Elements(maskCName) NE 0 THEN self.maskCName=maskCName ELSE self.maskCName = 'White'
...
IF N_Elements(contour_percent) NE 0 THEN contour_percent = 1B
...
IF Keyword_Set(maskColor) THEN RETURN, self.maskColor
IF Keyword_Set(maskCName) THEN RETURN, self.maskCName
...
IF Keyword_Set(smooth) THEN RETURN, self.smooth
IF Keyword_Set(smooth_type) THEN RETURN, self.smooth_type
IF Keyword_Set(smooth_radius) THEN RETURN, self.smooth_radius
...
IF Arg_Present(maskColor) THEN maskColor=self.maskColor
IF Arg_Present(maskCName) THEN maskCName=self.maskCName
...
If Arg_Present(fitParams) THEN BEGIN
    pixelsize = (*self.astr).cd[1,1] * (*self.astr).cdelt[1] ; in deg.
    fitParams=self.fitParams
pixelsize = 1.
    fitparams[2] = fitparams[2] * Abs(pixelsize)
    fitparams[3] = fitparams[3] * Abs(pixelsize)
    data = self->Convert_Coord(fitparams[4], fitparams[5], old_coord = 'Image')
    fitparams[4] = data[0]
    fitparams[5] = data[1]
    fitparams[6] = fitparams[6] * !radeg
ENDIF
If Arg_Present(fitError) THEN BEGIN
    pixelsize = (*self.astr).cd[1,1] ; in deg.
    fitError=self.fitError
    fitError[2] = fiterror[2] * Abs(pixelsize)
    fitError[3] = fiterror[3] * Abs(pixelsize)
    fitError[4] = fiterror[4] * Abs(float((*self.astr).cdelt[0]))
    fitError[5] = fiterror[5] * Abs(float((*self.astr).cdelt[0]))
ENDIF
If Arg_Present(tilt) THEN tilt=self.tilt

; DAOPhot parameters
IF Arg_Present(fwhm) THEN fwhm=self.fwhm
IF Arg_Present(err_fwhm) THEN err_fwhm=self.err_fwhm
...
; Smooth parameters
IF Arg_Present(smooth) THEN smooth=self.smooth
IF Arg_Present(smooth_type) THEN smooth_type=self.smooth_type
IF Arg_Present(smooth_radius) THEN smooth_radius=self.smooth_radius
...
ENDIF
IF Arg_Present(percent) THEN percent = self.contour_percent
...
IF N_Elements(maskColor) NE 0 THEN self.maskColor=maskColor
IF N_Elements(maskCName) NE 0 THEN self.maskCName=maskCName
IF N_Elements(minor) NE 0 THEN self.minor=minor
...
ENDIF
IF N_Elements(noimage) NE 0 THEN self.noimage=noimage
IF N_Elements(sexadecimal) NE 0 THEN self.sexadecimal=sexadecimal
IF N_Elements(subtitle) NE 0 THEN self.subtitle=subtitle
...
IF N_Elements(ticks) NE 0 THEN self.ticks=ticks
IF N_Elements(title) NE 0 THEN self.title=title
IF N_Elements(vel_unit) NE 0 THEN BEGIN
   poss_vel = ['pixel', 'm/s', 'km/s']
   IF total(StrLowCase(poss_vel) EQ vel_unit) EQ 1 THEN self.vel_unit = StrLowCase(vel_unit)
ENDIF
IF N_Elements(xtitle) NE 0 THEN self.xtitle=xtitle
IF N_Elements(ytitle) NE 0 THEN self.ytitle=ytitle
...
IF N_Elements(c_charsize) NE 0 THEN  *self.c_charsize = c_charsize[0]
IF N_Elements(c_charthick) NE 0 THEN *self.c_charthick = c_charthick[0]
...
IF N_Elements(percent) NE 0 THEN self.contour_percent= percent

IF N_Elements(smooth) NE 0 THEN BEGIN
    IF smooth NE self.smooth THEN BEGIN
        self.smooth = smooth
        self->Bytscl
    ENDIF
ENDIF
...
PRO Kang_Image::Limits, limits, coordinates = coordinates
; Sets the axes ranges for the image

; Convert to pixels
newlimits = self->Convert_Coord(limits[0:1], limits[2:3], old_coords = coordinates, new_coords = 'Image')
...
self.xsize = (self.pix_newlimits[1]-self.pix_newlimits[0]+1)
self.ysize = (self.pix_newlimits[3]-self.pix_newlimits[2]+1)
...
; ----------------------------------Fix Headers-----------------------------------
; Sometimes headers list '1' as naxis for dimensions 3 and 4 (pretty rare)
IF N_Elements(header) NE 0 AND size(header, /tname) EQ 'STRING' THEN BEGIN
   IF SXPar(header, 'NAXIS3') EQ 1 THEN BEGIN
      ...
      SXAddPar, header, 'NAXIS', SXPar(header, 'NAXIS')-1
   ENDIF
   IF SXPar(header, 'NAXIS4') EQ 1 THEN BEGIN
      SXDelPar, header, 'NAXIS4'
      SXDelPar, header, 'CDELT4'
      SXDelPar, header, 'CRPIX4'
      SXDelPar, header, 'CRVAL4'
      SXDelPar, header, 'CTYPE4'
      SXDelPar, header, 'CUNIT4'
      SXAddPar, header, 'NAXIS', SXPar(header, 'NAXIS')-1
   ENDIF
   IF SXPar(header, 'EQUINOX') EQ 0 THEN SXAddPar, header, 'EQUINOX', 2000
ENDIF
...
; if necessary
    self->ThreeD_Load, image, header, pointer=pointer, vel_astr = vel_astr
...
;IF N_Elements(image_size NE 2) THEN stop
...
; Store from cube
        IF N_Elements(vel_astr) NE 0 THEN BEGIN
           astr = *self.astr
           astr2 = {NAXIS:3, $
                    CD:astr.cd, $
                    CDELT:[astr.cdelt[0:1], vel_astr.cdelt], $
                    CRPIX:[astr.crpix[0:1], vel_astr.crpix], $
                    CRVAL:[astr.crval[0:1], vel_astr.crval], $
                    CTYPE:[astr.ctype[0:1], vel_astr.ctype], $
                    CUNIT:[['DEG', 'DEG'], vel_astr.ctype], $
...
IF noparams EQ -1 THEN BEGIN
   self.astr = Ptr_New(astr)
    ...
    IF N_Elements(image_size) EQ 3 THEN BEGIN
       self.image_type = 'Cube Array'
       *self.astr = {NAXIS:3, $
                     CD:[1, 1, 1], $
                     CDELT:[1, 1, 1], $
                     CRPIX:[1, 1, 1], $
                     CRVAL:[1, 1, 1], $
                     CTYPE:['IMAGE', 'IMAGE', 'IMAGE'], $
                     CUNIT:['PIX', 'PIX', 'PIX'], $
                     LONGPOLE:0, $
                     LATPOLE:0, $
                     PV2:[0, 0, 0]}
...
IF self.image_type EQ 'Cube' OR self.image_type EQ 'Cube Array' THEN BEGIN
    zmin=(self.pix_limits[4]+1 - (*self.astr).crpix[2]) * (*self.astr).cdelt[2] + (*self.astr).crval[2]
    zmax=(self.pix_limits[5]+1 - (*self.astr).crpix[2]) * (*self.astr).cdelt[2] + (*self.astr).crval[2]
ENDIF ELSE BEGIN
    zmin = 0
    zmax = 0
ENDELSE
...
;Constrain by cdelt and half the total
    IF N_Elements(deltavel) NE 0 THEN BEGIN
        deltavel = deltavel > (*self.astr).cdelt[2]
...
; Update the pixel limits
    IF self.image_type EQ 'Cube' OR self.image_type EQ 'Cube Array' THEN BEGIN
        pix_vel = Round((self.velocity - (*self.astr).crval[2])/(*self.astr).cdelt[2] + (*self.astr).crpix[2] - 1)
        pix_vellow = Round((self.newlimits[4] - (*self.astr).crval[2])/(*self.astr).cdelt[2] + (*self.astr).crpix[2] - 1)
        pix_velhigh = Round((self.newlimits[5] - (*self.astr).crval[2])/(*self.astr).cdelt[2] + (*self.astr).crpix[2] - 1)
...
PRO Kang_Image::ThreeD_Load, image, header, pointer=pointer, vel_astr = vel_astr
; This just transposes the arrays correctly

; Set velocity astr values so they are defined later
vel_astr = {cdelt:1., crpix:0., crval:0., ctype:'Image', cunit:'pixel'}

; Determine which axis is the velocity axis
IF N_Elements(header) NE 0 THEN BEGIN
   vel_arr = ['V', 'VEL', 'VRAD', 'VELOCITY', 'VELO', 'VELO_LSR', 'VELO-LSR']
...
; Rearrange the header
      ...
      SXAddPar, cunit, 'CUNIT3', cunit
   ENDIF
; set cube order
   self.cube_type = 'lbv'

; Some 3d headers have this stupid CD_3 extension
   IF vel_astr.cdelt EQ 0 Then vel_astr.cdelt = SXPar(header, 'CD3_3')
   
; Let's not have things backwards
   IF vel_astr.cdelt LT 0 THEN BEGIN
      vel_astr.cdelt = Abs(vel_astr.cdelt)
      image = Reverse(Temporary(image), 3)
   ENDIF

; Assume km/s by default
   IF StrTrim(cunit, 2) EQ '0' AND vel_astr.cdelt EQ 1 THEN vel_astr.cunit = 'pixel'
   IF StrTrim(cunit, 2) EQ '0' AND vel_astr.cdelt NE 1 THEN vel_astr.cunit = 'm/s'
   vel_astr.cdelt = StrTrim(StrLowCase(vel_astr.cdelt), 2)

; Convert to km/s
   IF vel_astr.cunit EQ 'm/s' THEN BEGIN
      vel_astr.cdelt = vel_astr.cdelt / 1000.
      vel_astr.crval = vel_astr.crval / 1000.
   ENDIF
ENDIF

END; Kang::ThreeD_Load-------------------------------------------------------


PRO Kang_Image::Smooth, radius, type = type

; Do the smoothing
threeD = self.pix_limits[4] EQ self.pix_limits[5] 
CASE CapFirstLetter(type) OF
  'Boxcar': BEGIN
      IF threeD THEN BEGIN
          *self.smooth_image = smooth(*self.image, radius, /NAN)
      ENDIF ELSE BEGIN
          *self.smooth_image = *self.image
          FOR i = 0, self.pix_limits[5] DO (*self.smooth_image)[*, *, i] = smooth((*self.image)[*, *, i], radius, /NAN)
      ENDELSE
  END
  ...
  'Median': BEGIN
      IF threeD THEN BEGIN
          *self.smooth_image = median(*self.image, radius, /Even)
      ENDIF ELSE BEGIN
          *self.smooth_image = *self.image
          FOR i = 0, self.pix_limits[5] DO (*self.smooth_image)[*, *, i] = median((*self.image)[*, *, i], radius, /Even)
      ENDELSE
  END

  'Gaussian': BEGIN

; Calculate size of gaussian filter.  This stolen from filter_image.pro in AstroLib
     npix = ( 3 * radius[ 0: ( (N_elements( radius )-1) < 1 ) ] ) > 3
     npix = 2 * fix( npix/2 ) + 1    ;make # pixels odd.
     box_wid = 0 > max( [npix] )

     filter = PSF_Gaussian(NP = npix, FWHM = radius, /NORM)

      IF threeD THEN BEGIN
          *self.smooth_image = convol(*self.image, filter, /NAN)
      ENDIF ELSE BEGIN
          *self.smooth_image = *self.image
          FOR i = 0, self.pix_limits[5] DO (*self.smooth_image)[*, *, i] = convol((*self.image)[*, *, i], filter, /NAN)
      ENDELSE
  END
...
; Store values, turn on smoothing
self.smooth = 1B
self.smooth_radius = radius
self.smooth_type = type
...
PRO Kang_Image::BytScl, lowScale, highScale, percent = percent
; Bytscales the image for display.
; Need to average if it is a cube and we are making an integrated intensity image.
; percent is just for storage

IF N_Elements(lowscale) EQ 0 THEN lowscale = self.lowscale
IF N_Elements(highscale) EQ 0 THEN highscale = self.highscale
...
IF self.smooth THEN BEGIN
    ...
    ENDIF ELSE BEGIN
        *self.byt_image = bytScl((*self.smooth_image)[*, *, self.pix_newlimits[4]], $
                                 Min=lowScale, $
                                 Max=highScale, $
                                 Top=self.NColors-1, /NAN)
    ENDELSE
...
IF N_Elements(percent) NE 0 THEN self.percent = percent ELSE self.percent = -1
...
IF self.pix_newlimits[4] EQ self.pix_newlimits[5] THEN BEGIN
    lowScale = Min((*self.image)[self.pix_newlimits[0]:self.pix_newlimits[1], $
                                 self.pix_newlimits[2]:self.pix_newlimits[3], self.pix_newlimits[4]], Max = highScale, /NAN)
    histvals = histogram((*self.image)[self.pix_newlimits[0]:self.pix_newlimits[1], $
                                       self.pix_newlimits[2]:self.pix_newlimits[3], self.pix_newlimits[4]], $
...
IF N_Elements(percent) NE 0 THEN BEGIN
    percentages = total(histvals, /Cumulative, /Double, /NAN) / Total(histvals, /Double, /NAN)
...
IF N_Elements(xpix) EQ 0 THEN xpix = round(self.pix_newlimits[0] + (self.pix_newlimits[1] - self.pix_newlimits[0]) /2.) ELSE xpix = round(xpix)
IF N_Elements(ypix) EQ 0 THEN ypix = round(self.pix_newlimits[2] + (self.pix_newlimits[3] - self.pix_newlimits[2]) /2.) ELSE ypix = round(ypix)
...
; Y is limiting, set y values
    pix_newlimits[2] = Ceil(ypix-(self.ysize)/(2.*factor))
    pix_newlimits[3] = Floor(pix_newlimits[2] + self.ysize / factor - 2)
...
; Determine how many x pixels we should show
    ...
    pix_newlimits[1] = pix_newlimits[0] + new_xsize - 1
;    pix_newlimits[0] = xpix-new_xsize/2.
;    pix_newlimits[1] = xpix+new_xsize/2.

ENDIF ELSE BEGIN
; X is limiting, set x values
    pix_newlimits[0] = Ceil(xpix-(self.xsize)/(2 * factor))
    pix_newlimits[1] = Floor(pix_newlimits[0] + self.xsize / factor - 2)
...
; Determine how many x pixels we should show
    ...
    pix_newlimits[3] = pix_newlimits[0] + new_xsize - 1
;    pix_newlimits[2] = ypix-new_ysize/2.
...
IF pix_newlimits[2] LT 0 THEN BEGIN
    ...
    pix_newlimits[3] = pix_newlimits[3] + offset+4
ENDIF ELSE BEGIN
...
IF Arg_Present(limits_out) THEN limits_out = pix_newlimits ELSE BEGIN
    self->Limits, pix_newlimits, coordinates = 'Image'
...
IF !D.Name EQ 'PS' THEN font = 2 ELSE font = self.font
plot, [0], $
;      XTicks = nx, $
;      YTicks = ny, $
...
PRO Kang_Image::Colorbar, scaling = scaling

IF self.highscale EQ self.lowscale THEN RETURN
...
;    IF scaling[0] EQ 'Log' THEN BEGIN
;        kang_find_levels, min = self.lowScale, max = self.highScale, /log, $
;                          tickv = tickv, tickname = tickname, ticks = ticks, levels = levels
;        divisions = ticks+1
...
colorbar, $
   ...
   charthick=self.charthick, $
   color=self.axiscolor, $
   divisions=5, $               ;divisions, $        ; Allows inexact ranges
   font=font, $
   format = '(I0)', $
;  format=format, $
...
PRO Kang_Image::TV, position = position, mask = mask
...
;one_pix_x = 1. / !D.X_Size
;one_pix_y = 1. / !D.Y_Size
;charsize_x = (!x.charsize > 1) * self.charsize
;charsize_y = (!y.charsize > 1) * self.charsize
;xmargin = (charsize_x * 26 + 15) * one_pix_x
;ymargin = (charsize_y * 26 + 10) * one_pix_y
;position[1] = ymargin
;position[0] = xmargin
; IF StrTrim(self.title, 2) EQ '' THEN self.position[3] = 1 - 2 * one_pix_y

; Determine pixels in mask
IF N_Elements(mask) EQ N_Elements(*self.byt_image) THEN $
   wh = where(mask[self.pix_newlimits[0]:self.pix_newlimits[1], self.pix_newlimits[2]:self.pix_newlimits[3]] NE 0, count) ELSE count = 0
IF count NE 0 THEN BEGIN
      byt_image = (*self.byt_image)[self.pix_newlimits[0]:self.pix_newlimits[1], self.pix_newlimits[2]:self.pix_newlimits[3]]

; Make masked regions green
      byt_image[wh] = self.maskColor
...
PRO Kang_Image::Draw, pix_newlimits = pix_newlimits, noaxis = noaxis, nobar = nobar, noimage = noimage, $
  position = position, noerase = noerase, scaling = scaling, mask = mask
...
IF N_Elements(noimage) EQ 0 THEN noimage = self.noimage
IF N_Elements(noaxis) EQ 0 THEN noaxis = self.noaxis
IF N_Elements(nobar) EQ 0 THEN nobar = self.nobar
;IF noaxis AND nobar THEN position = [0, 0, 1, 1]
...
; Draw image
IF ~Keyword_Set(noimage) THEN self->TV, position = position, mask = mask

; Colorbar
IF ~Keyword_Set(nobar) THEN self->Colorbar, scaling = scaling
...
;    hastrom, self->Integrate(), newheader, c_image, c_hdr, ref_header;, /silent
    hastrom, *self.image, newheader, c_image, c_hdr, ref_header, missing = !values.f_nan;, /silent

; If there is no overlap....
    IF N_Elements(c_image) EQ 0 THEN RETURN
ENDIF ELSE BEGIN
    IF self.smooth THEN BEGIN
        c_image = (*self.smooth_image)[self.pix_newlimits[0]:self.pix_newlimits[1], $
                  self.pix_newlimits[2]:self.pix_newlimits[3]]
    ENDIF ELSE BEGIN
        c_image = (*self.image)[self.pix_newlimits[0]:self.pix_newlimits[1], $
                  self.pix_newlimits[2]:self.pix_newlimits[3]]
    ENDELSE
ENDELSE
...
; Converting from pixels to values
    velocity_out = self.limits[4] > ((velocity_in +1 - astr.crpix[2]) * astr.cdelt[2] + astr.crval[2]) < self.limits[5]
ENDIF ELSE BEGIN
; Converting from values to pixels
    velocity_out = self.pix_limits[4] > Round((velocity_in - astr.crval[2]) / astr.cdelt[2] + astr.crpix[2] - 1)  < self.pix_limits[5]
ENDELSE
...
PRO Kang_Image::Velocity, velocity, velocity2, deltavel = deltavel, pixel = pixel
...
; Deltavel
IF N_Elements(deltavel) NE 0 THEN BEGIN
    IF Keyword_Set(pixel) THEN pix_deltavel = deltavel ELSE pix_deltavel = 2*Round(deltavel/(*self.astr).cdelt[2]/2.)
    self.deltavel = pix_deltavel*(*self.astr).cdelt[2]
ENDIF ELSE BEGIN
; No deltavel
    pix_deltavel = self.deltavel/(*self.astr).cdelt[2]
ENDELSE
...
xvals = scale_vector(findgen(n_vel), self.limits[4], self.limits[5])
IF (*self.astr).cunit[2] EQ 'm/s' THEN xvals = xvals / 1000.
...
PRO Kang_Image::Background, background = background, order = order, $
                            high_filter = high_filter, low_filter = low_filter, mask = mask
; Creates model backgrounds

; Default to third order polynomial
IF N_Elements(order) EQ 0 THEN order = 3

; Make 2D arrays of X and Y values
x = findgen(self.pix_limits[1]+1)
y = findgen(self.pix_limits[3]+1)
xx = x[*] # (y[*]*0 + 1)
yy = (x[*]*0 + 1) # y[*]

; Determine image values to use
IF N_Elements(mask) NE 0 THEN BEGIN
   wh = where(mask)
ENDIF ELSE BEGIN
   IF N_Elements(high_filter) NE 0 AND N_Elements(low_filter) NE 0 THEN $ ; Both set
      wh = where(finite(*self.image) AND *self.image LT high_filter AND *self.image GT low_filter)
   IF N_Elements(high_filter) NE 0 AND N_Elements(low_filter) EQ 0 THEN $ ; High set
      wh = where(finite(*self.image) AND *self.image LT high_filter)
   IF N_Elements(high_filter) EQ 0 AND N_Elements(low_filter) NE 0 THEN $ ; Low set
      wh = where(finite(*self.image) AND *self.image GT low_filter)
   IF N_Elements(high_filter) EQ 0 AND N_Elements(low_filter) EQ 0 THEN $ ; Neither set
      wh = where(finite(*self.image))
ENDELSE

; Convert to pixel locations
xy = array_indices([self.pix_limits[1]+1, self.pix_limits[3]+1], wh, /dim)

; Compute weights
weights = *self.image*0
weights[wh] = 1

; Array of coefficients.  Just guess they are all 1
p = replicate([1.], total(lindgen(order+1)+1))

; Let's get the first parameter right-ish though because otherwise it tends to "stick" at 1
a = mpfit2dfun('kang_poly2d', xx, yy, *self.image, 0, [1], weights=weights, $
               bestnorm=bestnorm, nfev=nfev, status=status, $
               parinfo=parinfo, perror=perror, niter=iter, $
               errmsg=errmsg, nfree=nfree, dof=dof, $
               functargs={order:0})
p[0] = a[0]

; Call fitting routine
a = mpfit2dfun('kang_poly2d', xx, yy, *self.image, 0, p, weights=weights, $
               bestnorm=bestnorm, nfev=nfev, status=status, $
               parinfo=parinfo, perror=perror, niter=iter, yfit=background, $
               errmsg=errmsg, nfree=nfree, dof=dof, $
               functargs={order:order})

; Convert back to image
IF ~Ptr_Valid(self.bg_image) THEN BEGIN
   self.bg_image = Ptr_New(background)
ENDIF ELSE BEGIN
   *self.bg_image = background
ENDELSE

END; Kang_Image::Background ------------------------------------------------


PRO Kang_Image::Subtract_Background
...
END; Kang_Image::Subtract_Background----------------------------------------
...
; Do the fitting
IF pix_newlimits[1]-pix_newlimits[0] GE 3 AND pix_newlimits[3]-pix_newlimits[2] GE 3 THEN BEGIN
   zfit = MPFit2DPeak((*self.image)[pix_newlimits[0] > 0:pix_newlimits[1] < self.pix_limits[1], $
                      ...
                      weights=weights, circular = circular, tilt = tilt, dof = dof, chisq=chisq, perror=error, /Quiet)

; Returns Gaussian sigma value, not FWHM

; Adjust values
   IF N_Elements(fitparams) NE 0 THEN BEGIN
      fitparams[4] = fitparams[4] + pix_newlimits[0] > 0
      fitparams[5] = fitparams[5] + pix_newlimits[2] > 0
...
PRO Kang_Image::Velocity_Field, output = output, velrange = velrange

FOR i = 0, self.pix_limits[5]-1 DO (*self.image)[*, *, i] = filter_image((*self.image)[*, *, i], fwhm=3, /all)

output = fltarr(self.pix_newlimits[1]-self.pix_newlimits[0]+1, self.pix_newlimits[3]-self.pix_newlimits[2]+1)

; Just take pix_newlimits for now
FOR i = self.pix_newlimits[0], self.pix_newlimits[1] DO BEGIN
   FOR j = self.pix_newlimits[2], self.pix_newlimits[3] DO BEGIN

; Create spectrum object
       yvals = self->Find_Values(i, j, lindgen(self.pix_limits[5]+1))
;       yvals = smooth(yvals, 5)
       xvals = scale_vector(findgen(self.pix_limits[5]), self.limits[4], self.limits[5])
       IF self.astr.cunit[2] EQ 'm/s' THEN xvals = xvals / 1000.
       bpix = 280
       epix = 315
       max = max(yvals[bpix:epix], max_loc)
       IF max GT 0.7 THEN output[i-self.pix_newlimits[0], j-self.pix_newlimits[2]] = (xvals[bpix:epix])[max_loc]
;       spec_obj = Obj_New('Kang_Spectrum', xvals, yvals)
;       spec_obj->Gauss, 1, xrange = velrange, /noclick
;       spec_obj->GetProperty, 
   ENDFOR
ENDFOR

END; Kang_Image::Velocity_Field------------------------------------------------------------
...
IF xlimits[0] LT xlimits[1] AND ((*self.astr).cdelt[0] LT 0 OR (*self.astr).cd[0,0] LT 0) THEN $
   xlimits = [xlimits[0], xlimits[1] - 360.]
;IF xlimits[1] GT xlimits[0] AND ((*self.astr).cdelt[0] GT 0 AND (*self.astr).cd[0,0] GT 0) THEN $
;   xlimits = [xlimits[0], xlimits[1]-360.]

; Store new limits
self.newlimits[0:3] = [xlimits, ylimits]

END; Kang_Image::Find_Newlimits----------------------------------------------------
...
;hextract, *self.image, header, image, hdr, $
;          self.pix_newlimits[0], $
;          self.pix_newlimits[1], $
;          self.pix_newlimits[2], $
;          self.pix_newlimits[3], $
;          self.pix_newlimits[4], $
;          self.pix_newlimits[5], $
;          /Silent

;image = (*self.image)[self.pix_newlimits[0]:self.pix_newlimits[1], self.pix_newlimits[2]:self.pix_newlimits[3], *]
...
FUNCTION Kang_Image::Find_Values, x, y, z, indices = indices, Data = data
...
IF N_Elements(indices) NE 0 THEN BEGIN
   xy = array_indices([self.pix_limits[1]+1, self.pix_limits[3]+1], indices, /Dimensions)
   x = reform(xy[0, *])
   y = reform(xy[1, *])
ENDIF
...
PRO Kang_Image::Find_Geometry, indices, min = min, max = max, centroid_min = centroid_min, centroid_max = centroid_max, $
                               median = median, mean = mean, n_nan = n_nan, stddev = stddev, weights = weights, sum = sum
...
IF N_Elements(indices) EQ 1 THEN BEGIN ; One index
    IF indices EQ -1 THEN BEGIN ; No indices
        ...
        n_nan = 0
    ENDIF ELSE BEGIN
        IF self.pix_newlimits[4] NE self.pix_newlimits[5] THEN min = (self->Integrate())[indices] ELSE $
          min = (*self.image)[indices]
        max = min
        centroid_min = array_indices([self.pix_limits[1]+1, self.pix_limits[3]+1], indices, /Dim)
...
; Multiple indices
    IF self.pix_newlimits[4] NE self.pix_newlimits[5] THEN image = (self->Integrate())[indices] ELSE $
      image = (*self.image)[indices]
    min = Min(image, whmin, Max = max, subscript_max = whmax, /NAN)
    centroid_min = array_indices([self.pix_limits[1]+1, self.pix_limits[3]+1], indices[whmin], /Dim)
    centroid_max = array_indices([self.pix_limits[1]+1, self.pix_limits[3]+1], indices[whmax], /Dim)
    binsize = (max-min)/(N_elements(image))^0.5

; Remove NANs
    good = where(finite(image), count, complement = bad)
    IF count GT 0 THEN indices = indices[good] ELSE indices = [-1]
    IF bad[0] NE -1 THEN n_nan = N_Elements(bad) ELSE n_nan = 0
...
; Weight the values. This could be flux or area weighted
       IF N_Elements(weights) NE 0 THEN BEGIN
          sum = total(*self.image * weights, /NAN)
          mean = sum / total(weights)
          result = moment((*self.image * weights)[indices], sDev = StdDev, /NAN, /Double)
          median = median((*self.image * weights)[indices], /Double)
       ENDIF ELSE BEGIN 
          result = moment(image, sDev = StdDev, /NAN, /Double)
          sum = total(image)
          mean = result[0]
          median = median(image, /Double)
       ENDELSE
    ENDELSE
ENDELSE
...
PRO Kang_Image::Histogram, indices, reverse_indices = reverse_indices, histvals=histvals, bins=bins, _Ref_Extra = extra
...
IF self.pix_newlimits[4] NE self.pix_newlimits[5] THEN BEGIN
   IF N_Elements(indices) EQ 0 THEN BEGIN
      histvals = histogram(self->Integrate(), locations = bins, nbins=nbins, reverse_indices = reverse_indices, /NAN, _Extra = extra)
   ENDIF ELSE BEGIN
      histvals = histogram((self->Integrate())[indices], locations = bins, nbins=nbins, reverse_indices = reverse_indices, /NAN, _Extra = extra)
   ENDELSE
ENDIF ELSE BEGIN
   IF N_Elements(indices) EQ 0 THEN BEGIN
      histvals = histogram((*self.image)[*, *, self.pix_newlimits[4]], locations = bins, nbins=nbins, reverse_indices = reverse_indices, /NAN, _Extra = extra)
   ENDIF ELSE BEGIN
      histvals = histogram(((*self.image)[*, *, self.pix_newlimits[4]])[indices], locations = bins, nbins=nbins, reverse_indices = reverse_indices, /NAN, _Extra = extra)
   ENDELSE
ENDELSE
...
; If we index a 2D array with pix_newlimits[4], we get too many values
   IF self.pix_newlimits[4] EQ 0 THEN BEGIN
      IF self.smooth THEN RETURN, (*self.smooth_image)[round(x), round(y)] ELSE $
         RETURN, (*self.image)[round(x), round(y)]
   ENDIF ELSE BEGIN
      IF self.smooth THEN RETURN, (*self.smooth_image)[round(x), round(y), self.pix_newlimits[4]] ELSE $
         RETURN, (*self.image)[round(x), round(y), self.pix_newlimits[4]]
   ENDELSE
ENDIF ELSE BEGIN
   IF self.smooth THEN RETURN, Total((*self.smooth_image)[round(x), round(y), self.pix_newlimits[4]:self.pix_newlimits[5]], 3, /NAN)/$
                               (self.pix_newlimits[5]-self.pix_newlimits[4]+1) $
   ELSE RETURN, Total((*self.image)[round(x), round(y), self.pix_newlimits[4]:self.pix_newlimits[5]], 3, /NAN)/$
                (self.pix_newlimits[5]-self.pix_newlimits[4]+1)
ENDELSE

END ;Kang_Image::Integrate----------------------------------------------------
...
IF self.pix_newlimits[4] EQ self.pix_newlimits[5] THEN $
   indices = search2D((*self.image)[*, *, self.pix_newlimits[4]], xclick, yclick, threshold_low, threshold_high) $
ELSE $
...
PRO Kang_Image::Transpose, lbv = lbv, bvl = bvl, vbl = vbl, lvb = lvb, vlb = vlb

; Transpose the array so it is lbv
IF Keyword_Set(lbv) THEN BEGIN
   arr = [StrPos(self.cube_type, 'l'), StrPos(self.cube_type, 'b'), StrPos(self.cube_type, 'v')]
   self.cube_type = 'lbv'
ENDIF
IF Keyword_Set(bvl) THEN BEGIN
   arr = [StrPos(self.cube_type, 'b'), StrPos(self.cube_type, 'v'), StrPos(self.cube_type, 'l')]
   self.cube_type = 'bvl'
ENDIF
IF Keyword_Set(vbl) THEN BEGIN
   arr = [StrPos(self.cube_type, 'v'), StrPos(self.cube_type, 'b'), StrPos(self.cube_type, 'l')]
   self.cube_type = 'vbl'
ENDIF
IF Keyword_Set(lvb) THEN BEGIN
   arr = [StrPos(self.cube_type, 'l'), StrPos(self.cube_type, 'v'), StrPos(self.cube_type, 'b')]
   self.cube_type = 'lvb'
ENDIF
IF Keyword_Set(vlb) THEN BEGIN
   arr = [StrPos(self.cube_type, 'v'), StrPos(self.cube_type, 'l'), StrPos(self.cube_type, 'b')]
   self.cube_type = 'vlb'
ENDIF

; Transpose image
*self.image = Transpose(Temporary(*self.image), arr)

; Transpose limits arrays
limits_arr = [arr[0]*2, arr[0]*2+1, arr[1]*2, arr[1]*2+1, arr[2]*2, arr[2]*2+1]
self.pix_limits = self.pix_limits[limits_arr]
self.pix_newlimits = self.pix_limits
self.limits = self.limits[limits_arr]
self.newlimits = self.limits
...
; Change velocity stuff

END; Kang_Image::Transpose--------------------------------------------------


PRO Kang_Image::Aper, flux = flux

; Determine limits for fitting
fwhm = self.fwhm*2
fitparams_arr = fltarr((*self.stars).n_stars, 7)
star_limits = [[(*self.stars).x - fwhm], [(*self.stars).x + fwhm], [(*self.stars).y - fwhm], [(*self.stars).y + fwhm]]        
FOR i = 0, (*self.stars).n_stars-1 DO BEGIN
    IF (*self.stars).good[i] THEN BEGIN
        zfit = self->Fit2DPeak(fitparams, error, pix_newlimits = star_limits[i, *], fitType = 'Gaussian', /tilt)
        fitparams_arr[i, *] = fitparams
    ENDIF
ENDFOR

; Assume Gaussian shape here, remove baseline
(*self.stars).flux = 2*!pi*fitparams_arr[*, 1] * fitparams_arr[*, 2] * fitparams_arr[*, 3] - fitparams_arr[*, 0]
flux = (*self.stars).flux

;aper, *self.image, (*self.stars).x, (*self.stars).y, mags, errap, sky, skyerr, 1, [3, 5, 7], [9, 13], /NAN, /Silent
;stop
END; Kang_Image::Aper-------------------------------------------------------


PRO Kang_Image::Convol, fwhm
; Convolves the image with a Gaussian of FWHM fwhm in pixels
; Stolen completely from find.pro in the AstroLib and rearranged

IF N_Elements(fwhm) EQ 0 THEN BEGIN
    Print, 'Please specify a FWHM.'
    RETURN
ENDIF
IF fwhm EQ self.fwhm THEN BEGIN
    Print, 'FWHM equals that of previously run convolution - resulting image unchanged.'
    RETURN
ENDIF
...
self.nhalf = fix(radius) < (maxbox-1)/2 ; # of pixels halfway across convolution box
self.nbox = 2*self.nhalf + 1    ;number of pixels in side of convolution box 

; Compute convolution kernel gauss_kernel
gauss_kernel = fltarr(self.nbox, self.nbox)
row2 = (findgen(self.nbox)-self.nhalf)^2
FOR i = 0, self.nhalf DO BEGIN
    temp = row2 + i^2
    gauss_kernel[0, self.nhalf-i] = temp         
    gauss_kernel[0, self.nhalf+i] = temp                           
ENDFOR

; Mask out pixels from convolution filter that are >radius^2
; kernel_mask is complementary to SKIP in Stetson's Fortran
IF Ptr_Valid(self.kernel_mask) THEN *self.kernel_mask = fix(gauss_kernel LE radius^2.) ELSE $
  self.kernel_mask = Ptr_New(fix(gauss_kernel LE radius^2.))
good = where(*self.kernel_mask, n_good) ;Value of "kernel" are now equal to distance to center

; Compute real convolution kernel "self.kernel"
sigsq = (fwhm/2.35482)^2
gauss_kernel = exp(-0.5*gauss_kernel/sigsq)
IF Ptr_Valid(self.kernel) THEN *self.kernel = gauss_kernel*(*self.kernel_mask) ELSE $
  self.kernel = Ptr_New(gauss_kernel*(*self.kernel_mask))

; Normalize kernel
sum_kernel = total(*self.kernel)
sum_kernelsq = total(*self.kernel^2) - sum_kernel^2/n_good
sum_kernel = sum_kernel/n_good
...
IF Ptr_Valid(self.convolved_image) THEN BEGIN
   IF self.pix_newlimits[4] EQ self.pix_newlimits[5] THEN BEGIN
      *self.convolved_image = convol((*self.image)[*,*,self.pix_newlimits[4]], *self.kernel)
   ENDIF
ENDIF ELSE BEGIN
   IF self.pix_newlimits[4] EQ self.pix_newlimits[5] THEN BEGIN
      self.convolved_image = Ptr_New(convol((*self.image)[*,*,self.pix_newlimits[4]], *self.kernel))
   ENDIF
ENDELSE

; Blank out edges
(*self.convolved_image)[0:self.nhalf-1,*] = 0
(*self.convolved_image)[self.pix_limits[1]+1-self.nhalf:self.pix_limits[1], *] = 0
(*self.convolved_image)[*,0:self.nhalf-1] = 0
(*self.convolved_image)[*, self.pix_limits[3]+1-self.nhalf:self.pix_limits[3]] = 0
...
; Define symsize in relation to number of pixels displayed
    ...
    IF bad[0] NE -1 THEN $
      oplot, ((*self.stars).x)[bad], ((*self.stars).y)[bad], psym = 8, symsize = symsize, color = bad_color
...
; Locate local maxima
(*self.kernel_mask)[self.nhalf, self.nhalf] = 0  ;From now on we exclude the central pixel
good = where(*self.kernel_mask, pixels)      ;"good" identifies position of valid pixels
xx= (good mod self.nbox) - self.nhalf ;x and y coordinate of valid pixels 
yy = fix(good/self.nbox) - self.nhalf ;relative to the center
...
xwt = fltarr(self.nbox, self.nbox)
wt = self.nhalf - abs(findgen(self.nbox)-self.nhalf ) + 1
FOR i = 0, self.nbox-1 DO xwt[0, i] = wt
ywt = transpose(xwt)
...
vec = self.nhalf - findgen(self.nbox) 
dgdx = sgy*vec
dgdy = sgx*vec
...
FOR i = 0L, n_stars-1 DO BEGIN
    temp = float((*self.image)[(ix[i]-self.nhalf) > 0:(ix[i]+self.nhalf) < self.pix_limits[1], $
                               (iy[i]-self.nhalf) > 0:(iy[i]+self.nhalf) < self.pix_limits[3]])

; Compute Sharpness statistic
    sharpness[i] = (temp[self.nhalf, self.nhalf] - (total(*self.kernel_mask*temp))/pixels)/flux[i]
...
self.stars = Ptr_New({fwhm:self.fwhm, $
                      n_stars:n_stars, $
                      x:x, $
                      y:y, $
;                      long:long, $
;                      lat:lat, $
                      ...
                      good:bytarr(n_stars), $
                      n_good:0L, $
                      indices:lindgen(n_stars), $
                      sort_type:'None'}) ; indices are the sorting indices into the x, y, etc. arrays

END; Kang_Image::Find-------------------------------------------------------


PRO Kang_Image::Filter, sharplim = sharplim, roundlim = roundlim, centroidlim = centroidlim
; Find bad stars
...
IF N_Elements(centroidlim) NE 2 THEN centroidlim = self.nhalf
...
good_in = where(((*self.stars).sharpness GT sharplim[0]) AND ((*self.stars).sharpness LT sharplim[1]) AND $
                ...
                ((*self.stars).hx GT 0 AND abs((*self.stars).dx2) LT self.nhalf AND $
                 (*self.stars).hy GT 0 AND abs((*self.stars).dy2) LT self.nhalf), n_good)

; Store good ones
good = BytArr((*self.stars).n_stars)
IF good_in[0] NE -1 THEN good[good_in] = 1B
...
; Determine limits for fitting
fwhm = self.fwhm*2
star_limits = [(*self.stars).x - fwhm, (*self.stars).y + fwhm, (*self.stars).x - fwhm, (*self.stars).y + fwhm]        
...
PRO Kang_Image::Sky, skyval, skysig, CIRCLERAD = circlerad, MEANBACK = meanback, _EXTRA = extra
  
; Thsi is crashing more than I would expect - this code hides the crashes but ideally they wouldn't occur
Catch, theError
...
; Determine min and max in displayed image
IF self.pix_newlimits[4] EQ self.pix_newlimits[5] THEN BEGIN
    sky, (*self.image)[*,*,self.pix_newlimits[4]], skyval, skysig, CIRCLERAD = circlerad, _EXTRA = extra, /NAN, MEANBACK = meanback, /Silent
ENDIF ELSE BEGIN
    sky, self->Integrate(), skyval, skysig, CIRCLERAD = circlerad, _EXTRA = extra, /NAN, MEANBACK = meanback, /Silent
ENDELSE
...
PRO Kang_Image::Sort_Stars, x = x, y = y, flux = flux, sharpness = sharpness, roundness = roundness

IF ~Ptr_Valid(self.stars) THEN BEGIN
    print, 'No valid stars found yet.  Please run "find" first.'
    RETURN
ENDIF

IF Keyword_Set(x) THEN BEGIN
   (*self.stars).indices = sort((*self.stars).x)
   IF (*self.stars).sort_type EQ 'x' THEN (*self.stars).indices = reverse((*self.stars).indices) ELSE (*self.stars).sort_type = 'x'
ENDIF
IF Keyword_Set(y) THEN BEGIN
   (*self.stars).indices = sort((*self.stars).y)
   IF (*self.stars).sort_type EQ 'y' THEN (*self.stars).indices = reverse((*self.stars).indices) ELSE (*self.stars).sort_type = 'y'
ENDIF
IF Keyword_Set(flux) THEN BEGIN
   (*self.stars).indices = sort((*self.stars).flux)
   IF (*self.stars).sort_type EQ 'flux' THEN (*self.stars).indices = reverse((*self.stars).indices) ELSE (*self.stars).sort_type = 'flux'
ENDIF
IF Keyword_Set(sharpness) THEN BEGIN
   (*self.stars).indices = sort((*self.stars).sharpness)
   IF (*self.stars).sort_type EQ 'sharpness' THEN (*self.stars).indices = reverse((*self.stars).indices) ELSE (*self.stars).sort_type = 'sharpness'
ENDIF
IF Keyword_Set(roundness) THEN BEGIN
   (*self.stars).indices = sort((*self.stars).roundness)
   IF (*self.stars).sort_type EQ 'roundness' THEN (*self.stars).indices = reverse((*self.stars).indices) ELSE self.sort_stars = 'roundness'
ENDIF

END; Kang_Image::Sort_Stars----------------------------------------------------
...
struct = {KANG_IMAGE, $
          ...
          smooth_image:Ptr_New(), $ ; A pointer to a smoothed copy of the image.
          bg_image:Ptr_New(), $     ; A pointer to the model background
          header:Ptr_New(), $       ; A pointer to the image header
          newheader:Ptr_New(), $    ; A copy of the header used to make subimages
          image_type:'', $          ; Can be cube, etc
...
; Plot parameters
          ...
          xsize:0., $                               ; The size of the image after pix_newlimits has been applied
          ysize:0., $
...
; Smoothing
          smooth:0B, $          ; Should we use the smoothed image?
          smooth_type:'', $     ; Can be boxcar, median, or gaussian now 
          smooth_radius:0., $   ; Radius to smooth over
